...
// Sourced from src/js/internal/cluster/RoundRobinHandle.ts


const { append, init, isEmpty, peek, remove } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 27/*internal/linkedlist*/) || __intrinsic__createInternalModuleById(27/*internal/linkedlist*/));
...
$ = class RoundRobinHandle {
  ...
  constructor(key, address, { port, fd, flags, backlog, readableAll, writableAll }) {
    net ??= (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 104/*node:net*/) || __intrinsic__createInternalModuleById(104/*node:net*/));
    this.key = key;
    this.all = new Map();
    this.free = new Map();
