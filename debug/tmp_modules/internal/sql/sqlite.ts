...
// Sourced from src/js/internal/sql/sqlite.ts
...
const { SQLHelper, SQLResultArray, buildDefinedColumnsAndQuery } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 37/*internal/sql/shared*/) || __intrinsic__createInternalModuleById(37/*internal/sql/shared*/));
const {
  Query,
  SQLQueryResultMode,
...
const { SQLiteError } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 33/*internal/sql/errors*/) || __intrinsic__createInternalModuleById(33/*internal/sql/errors*/));

let lazySQLiteModule: typeof BunSQLiteModule;
function getSQLiteModule() {
  if (!lazySQLiteModule) {
    lazySQLiteModule = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 2/*bun/sqlite.ts*/) || __intrinsic__createInternalModuleById(2/*bun/sqlite.ts*/));
  }
  ...
  run(db: BunSQLiteModule.Database, query: Query<any, any>) {
    ...
    try {
      ...
      if (parsedInfo.canReturnRows) {
        ...
        const sqlResult = __intrinsic__isArray(result) ? new SQLResultArray(result) : new SQLResultArray([result]);

        sqlResult.command = commandToString(command, parsedInfo.lastToken);
        sqlResult.count = __intrinsic__isArray(result) ? result.length : 1;

        stmt.finalize();
        query.resolve(sqlResult);
      } else {
        // For INSERT/UPDATE/DELETE/CREATE etc., use db.run() which handles multiple statements natively
        const changes = db.run.__intrinsic__apply(db, [sql].concat(values));
