...
// Sourced from src/js/internal/fs/streams.ts
import type { FileSink } from "bun"

// fs.ReadStream and fs.WriteStream are lazily loaded to avoid importing 'node:stream' until required
;
const { Readable, Writable, finished } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117/*node:stream*/) || __intrinsic__createInternalModuleById(117/*node:stream*/));
const fs: typeof import("node:fs") = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 98/*node:fs*/) || __intrinsic__createInternalModuleById(98/*node:fs*/));
const { read, write, fsync, writev } = fs;
const { FileHandle, kRef, kUnref, kFd } = (fs.promises as any).__intrinsic__data as {
...
const { validateInteger, validateInt32, validateFunction } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68/*internal/validators*/) || __intrinsic__createInternalModuleById(68/*internal/validators*/));
...
function streamFileHandleClose(this: FileHandle, fd: FD, cb: (err?: any) => void) {
  !(IS_BUN_DEVELOPMENT?$assert(this[kFd] == fd,"this[kFd] == fd", "fd mismatch"):void 0);
  this[kUnref]();
...
function ReadStream(this: FSStream, path, options): void {
  ...
  !(IS_BUN_DEVELOPMENT?$assert(this[kFs],"this[kFs]", "fs implementation was not assigned"):void 0);
...
readStreamPrototype._read = function (n) {
  n = this.pos !== undefined ? __intrinsic__min(this.end - this.pos + 1, n) : __intrinsic__min(this.end - this.bytesRead + 1, n);
  ...
  this[kFs].read(this.fd, buf, 0, n, this.pos, (er, bytesRead, buf) => {
    if (er) {
      (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 43/*internal/streams/destroy*/) || __intrinsic__createInternalModuleById(43/*internal/streams/destroy*/)).errorOrDestroy(this, er);
    } else if (bytesRead > 0) {
...
function WriteStream(this: FSStream, path: string | null, options?: any): void {
  ...
  if (customFs) {
    ...
    if (!write && !writev) {
      throw __intrinsic__makeErrorWithCode(118, "options.fs.write", "function", write);
    }
  } else {
    this._writev = undefined;
    !(IS_BUN_DEVELOPMENT?$assert(this[kFs].write,"this[kFs].write", "assuming user does not delete fs.write!"):void 0);
  }

  if (flush == null) {
    this.flush = false;
  } else {
    if (typeof flush !== "boolean") throw __intrinsic__makeErrorWithCode(118, "options.flush", "boolean", flush);
    this.flush = flush;
  }
...
function writevAll(chunks, size, pos, cb, retries = 0) {
  this[kFs].writev(this.fd, chunks, this.pos, (er, bytesWritten, buffers) => {
    ...
    if (retries > 5) {
      ...
      writevAll.__intrinsic__call(this, [Buffer.concat(buffers).slice(bytesWritten)], size, pos, cb, retries);
    } else {
      cb();
    }
  });
...
function writeFast(this: FSStream, data: any, encoding: any, cb: any) {
  if (this[kWriteMonkeyPatchDefense]) return writablePrototypeWrite.__intrinsic__call(this, data, encoding, cb);
...
function thenIfPromise<T>(maybePromise: Promise<T> | T, cb: any) {
  !(IS_BUN_DEVELOPMENT?$assert(typeof cb === "function","typeof cb === \"function\"", "cb is not a function"):void 0);
  if (__intrinsic__isPromise(maybePromise)) {
...
function writableFromFileSink(fileSink: any) {
  !(IS_BUN_DEVELOPMENT?$assert(typeof fileSink === "object","typeof fileSink === \"object\"", "fileSink is not an object"):void 0);
  !(IS_BUN_DEVELOPMENT?$assert(typeof fileSink.write === "function","typeof fileSink.write === \"function\"", "fileSink.write is not a function"):void 0);
  !(IS_BUN_DEVELOPMENT?$assert(typeof fileSink.end === "function","typeof fileSink.end === \"function\"", "fileSink.end is not a function"):void 0);
  const w = new WriteStream("", { __intrinsic__fastPath: true });
  !(IS_BUN_DEVELOPMENT?$assert(w[kWriteStreamFastPath] === true,"w[kWriteStreamFastPath] === true", "fast path not enabled"):void 0);
