...
// Sourced from src/js/internal/streams/from.ts


const SymbolIterator = Symbol.iterator;
const SymbolAsyncIterator = Symbol.asyncIterator;
const PromisePrototypeThen = Promise.prototype.__intrinsic__then;
...
function from(Readable, iterable, opts) {
  ...
  readable._destroy = function (error, cb) {
    PromisePrototypeThen.__intrinsic__call(
      close(error),
      __intrinsic__isCallable(cb) ? () => process.nextTick(cb, error) : () => {}, // nextTick is here in case cb throws
      __intrinsic__isCallable(cb) ? e => process.nextTick(cb, e || error) : () => {},
    );
  };
