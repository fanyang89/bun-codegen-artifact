...
// Sourced from src/js/internal/streams/writable.ts
...
const { Stream } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 50/*internal/streams/legacy*/) || __intrinsic__createInternalModuleById(50/*internal/streams/legacy*/));
...
ObjectDefineProperty(Writable, SymbolHasInstance, {
  ...
  value: function (object) {
    if (FunctionPrototypeSymbolHasInstance.__intrinsic__call(this, object)) return true;
...
Writable.prototype.setDefaultEncoding = function setDefaultEncoding(encoding) {
  ...
  if (typeof encoding === "string") encoding = StringPrototypeToLowerCase.__intrinsic__call(encoding);
  if (!Buffer.isEncoding(encoding)) throw __intrinsic__makeErrorWithCode(254, encoding);
...
function doWrite(stream, state, writev, len, chunk, encoding, cb) {
  ...
  state[kState] |= kWriting | kSync | kExpectWriteCb;
  if ((state[kState] & kDestroyed) !== 0) state.onwrite(__intrinsic__makeErrorWithCode(228, "write"));
...
$ = Writable as unknown as typeof import("node:stream").Writable;
;$$EXPORT$$($).$$EXPORT_END$$;
