...
const bindings = __intrinsic__lazy(95);
...
class Response extends WebResponse {
  ...
  constructor(body, init) {
    const { Readable, Stream } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117/*node:stream*/) || __intrinsic__createInternalModuleById(117/*node:stream*/));
  ...
  get body() {
    ...
    if (!body) {
      var web = super.body;
      if (!web) return null;
      body = this[kBody] = new ((__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 69/*internal/webstreams_adapters*/) || __intrinsic__createInternalModuleById(69/*internal/webstreams_adapters*/))._ReadableFromWeb)({}, web);
    }
  ...
  async buffer() {
    // load the getter
    void this.body;
    return new __intrinsic__Buffer(await super.arrayBuffer());
  }
...
async function fetch(
  ...
  if (init?.body && typeof init.body === "object" && !init.body[Symbol.asyncIterator]) {
    const { Readable, Stream, PassThrough } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117/*node:stream*/) || __intrinsic__createInternalModuleById(117/*node:stream*/));
  ...
  const response = await nativeFetch.__intrinsic__call(undefined, url, init);
