// @bun
// build/debug/tmp_modules/thirdparty/node-fetch.ts
var $;
var bindings = __intrinsic__lazy(95);
...
class Response extends WebResponse {
  ...
  constructor(body, init) {
    const { Readable, Stream } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117) || __intrinsic__createInternalModuleById(117);
...
async function fetch(url, init) {
  if (init?.body && typeof init.body === "object" && !init.body[Symbol.asyncIterator]) {
    const { Readable, Stream, PassThrough } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117) || __intrinsic__createInternalModuleById(117);
  ...
  const response = await nativeFetch.__intrinsic__call(__intrinsic__undefined, url, init);
