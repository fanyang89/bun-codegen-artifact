// @bun
// build/debug/tmp_modules/internal/streams/pipeline.ts
var $;
var eos = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 47) || __intrinsic__createInternalModuleById(47);
var { once } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 32) || __intrinsic__createInternalModuleById(32);
...
var { validateFunction, validateAbortSignal } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68);
var {
  isIterable,
  isReadable,
...
async function* fromReadable(val) {
  Readable ??= __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 55) || __intrinsic__createInternalModuleById(55);
...
async function pumpToNode(iterable, writable, finish, { end }) {
  ...
  const wait = () => new __intrinsic__Promise((resolve, reject) => {
    if (error) {
      reject(error);
    } else {
      onresolve = () => {
    ...
    if (i === 0) {
      ...
      if (reading) {
        ...
        if (typeof then === "function") {
          finishCount++;
          then.__intrinsic__call(ret, (val) => {
            value = val;
            if (val != null) {
              pt.write(val);
            }
