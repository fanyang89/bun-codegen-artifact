// @bun
// build/debug/tmp_modules/internal/streams/duplexify.ts
var $;
var {
  isReadable,
  isWritable,
...
function duplexify(body, name) {
  ...
  if (typeof body === "function") {
    ...
    if (typeof then2 === "function") {
      let d;
      const promise = then2.__intrinsic__call(value, (val) => {
        if (val != null) {
  ...
  if (typeof body?.writable === "object" || typeof body?.readable === "object") {
    const readable = body?.readable ? isReadableNodeStream(body?.readable) ? body?.readable : duplexify(body.readable) : __intrinsic__undefined;
    const writable = body?.writable ? isWritableNodeStream(body?.writable) ? body?.writable : duplexify(body.writable) : __intrinsic__undefined;
    return _duplexify({ readable, writable });
  }
  const then = body?.then;
  if (typeof then === "function") {
    let d;
    then.__intrinsic__call(body, (val) => {
      if (val != null) {
        d.push(val);
      }
      d.push(null);
