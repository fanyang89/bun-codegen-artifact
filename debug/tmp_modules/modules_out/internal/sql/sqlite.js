// @bun
// build/debug/tmp_modules/internal/sql/sqlite.ts
var $;
...
var { SQLiteError } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 33) || __intrinsic__createInternalModuleById(33);
  ...
  run(db, query) {
    ...
    try {
      ...
      if (err && typeof err === "object" && "name" in err && err.name === "SQLiteError") {
        const code = "code" in err ? __intrinsic__String(err.code) : "SQLITE_ERROR";
        const errno = "errno" in err ? Number(err.errno) : 1;
        const byteOffset = "byteOffset" in err ? Number(err.byteOffset) : __intrinsic__undefined;
        const message = "message" in err ? __intrinsic__String(err.message) : "SQLite error";
