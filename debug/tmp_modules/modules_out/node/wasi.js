// @bun
// build/debug/tmp_modules/node/wasi.ts
var $;
var nodeFsConstants = __intrinsic__processBindingConstants.fs;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __commonJS = (cb, mod = __intrinsic__undefined) => function __require2() {
        ...
        this.wasiImport = {
          args_get: (argv, argvBuf) => {
            ...
            args.forEach((a) => {
              ...
              coffset += 4;
              offset += __intrinsic__Buffer.from(this.memory.buffer).write(`${a}\x00`, offset);
            });
          ...
          args_sizes_get: (argc, argvBufSize) => {
            ...
            const size = args.reduce((acc, a) => acc + __intrinsic__Buffer.byteLength(a) + 1, 0);
          ...
          environ_get: (environ, environBuf) => {
            ...
            Object.entries(this.env).forEach(([key, value]) => {
              this.view.setUint32(coffset, offset, true);
              coffset += 4;
              offset += __intrinsic__Buffer.from(this.memory.buffer).write(`${key}=${value}\x00`, offset);
            });
            return constants_1.WASI_ESUCCESS;
          },
          environ_sizes_get: (environCount, environBufSize) => {
            this.refreshMemory();
            const envProcessed = Object.entries(this.env).map(([key, value]) => `${key}=${value}\x00`);
            const size = envProcessed.reduce((acc, e) => acc + __intrinsic__Buffer.byteLength(e), 0);
          ...
          fd_prestat_get: wrap((fd, bufPtr) => {
            ...
            this.view.setUint32(bufPtr + 4, __intrinsic__Buffer.byteLength(stats.fakePath ?? stats.path ?? ""), true);
            return constants_1.WASI_ESUCCESS;
          }),
          fd_prestat_dir_name: wrap((fd, pathPtr, pathLen) => {
            const stats = CHECK_FD(fd, BigInt(0));
            this.refreshMemory();
            __intrinsic__Buffer.from(this.memory.buffer).write(stats.fakePath ?? stats.path ?? "", pathPtr, pathLen, "utf8");
            return constants_1.WASI_ESUCCESS;
          }),
          ...
          fd_readdir: wrap((fd, bufPtr, bufLen, cookie, bufusedPtr) => {
            ...
            for (let i = Number(cookie);i < entries.length; i += 1) {
              ...
              let nameLength = __intrinsic__Buffer.byteLength(entry.name);
              ...
              let memory_buffer = __intrinsic__Buffer.from(this.memory.buffer);
          ...
          path_create_directory: wrap((fd, pathPtr, pathLen) => {
            ...
            this.refreshMemory();
            const p = __intrinsic__Buffer.from(this.memory.buffer, pathPtr, pathLen).toString();
          ...
          path_link: wrap((oldFd, _oldFlags, oldPath, oldPathLen, newFd, newPath, newPathLen) => {
            ...
            this.refreshMemory();
            const op = __intrinsic__Buffer.from(this.memory.buffer, oldPath, oldPathLen).toString();
            const np = __intrinsic__Buffer.from(this.memory.buffer, newPath, newPathLen).toString();
          ...
          path_readlink: wrap((fd, pathPtr, pathLen, buf, bufLen, bufused) => {
            ...
            const used = __intrinsic__Buffer.from(this.memory.buffer).write(r, buf, bufLen);
