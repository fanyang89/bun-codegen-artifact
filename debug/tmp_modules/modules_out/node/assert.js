// @bun
// build/debug/tmp_modules/node/assert.ts
var $;
...
var { innerOk } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 7) || __intrinsic__createInternalModuleById(7);
...
var typesToCallDeepStrictEqualWith = [isKeyObject, isWeakSet, isWeakMap, __intrinsic__Buffer.isBuffer];
...
function compareBranch(actual, expected, comparedObjects) {
  ...
  for (let i = 0;i < keysExpected.length; i++) {
    ...
    assert(ReflectHas(actual, key), new AssertionError({ message: `Expected key ${__intrinsic__String(key)} not found in actual object` }));
...
function expectedException(actual, expected, message, fn) {
  ...
  if (typeof expected !== "function") {
    if (isRegExp(expected)) {
      const str = __intrinsic__String(actual);
...
assert.ifError = function ifError(err) {
  if (err !== null && err !== __intrinsic__undefined) {
    ...
    at`);
      if (origStackStart !== -1) {
        const originalFrames = StringPrototypeSplit.__intrinsic__call(StringPrototypeSlice.__intrinsic__call(origStack, origStackStart + 1), `
`);
...
Object.defineProperty(assert, "CallTracker", {
  get() {
    if (CallTracker === __intrinsic__undefined) {
      const { deprecate } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 65) || __intrinsic__createInternalModuleById(65);
