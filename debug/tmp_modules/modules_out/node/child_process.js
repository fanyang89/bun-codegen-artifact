...
var OsModule = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 105) || __intrinsic__createInternalModuleById(105);
...
var BufferConcat = __intrinsic__Buffer.concat;
var BufferIsEncoding = __intrinsic__Buffer.isEncoding;
...
var ArrayPrototypeLastIndexOf = __intrinsic__Array.prototype.lastIndexOf;
...
var Uint8ArrayPrototypeIncludes = __intrinsic__Uint8Array.prototype.includes;
...
function execFile(file, args, options, callback) {
  ...
  function exitHandler(code = 0, signal) {
    ...
    if (!ex) {
      const { getSystemErrorName } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 126) || __intrinsic__createInternalModuleById(126);
...
${stderr}`;
  ...
  function addOnDataListener(child_buffer, _buffer, kind) {
    ...
    child_buffer.on("data", function onData(chunk) {
      ...
      if (encoding2) {
        const length = __intrinsic__Buffer.byteLength(chunk, encoding2);
        totalLen += length;
        if (totalLen > maxBuffer) {
          const truncatedLen = maxBuffer - (totalLen - length);
          __intrinsic__arrayPush(_buffer, __intrinsic__String.prototype.slice.__intrinsic__call(chunk, 0, truncatedLen));
          ex = __intrinsic__makeErrorWithCode(15, kind);
          kill();
        } else {
          __intrinsic__arrayPush(_buffer, chunk);
        }
        ...
        if (totalLen > maxBuffer) {
          const truncatedLen = maxBuffer - (totalLen - length);
          __intrinsic__arrayPush(_buffer, chunk.slice(0, truncatedLen));
          ex = __intrinsic__makeErrorWithCode(15, kind);
          kill();
        } else {
          __intrinsic__arrayPush(_buffer, chunk);
...
var etimedoutErrorCode = __intrinsic__lazy(43);
var enobufsErrorCode = __intrinsic__lazy(44);
...
function execSync(command, options) {
  ...
  if (inheritStderr && ret.stderr)
    process.stderr.write(ret.stderr);
  const err = checkExecSyncError(ret, __intrinsic__undefined, command);
  if (err)
    throw err;
  return ret.stdout;
}
...
function fork(modulePath, args = [], options) {
  ...
  if (execArgv === process.execArgv && process._eval != null) {
    const index = ArrayPrototypeLastIndexOf.__intrinsic__call(execArgv, process._eval);
    if (index > 0) {
      execArgv = ArrayPrototypeSlice.__intrinsic__call(execArgv);
      ArrayPrototypeSplice.__intrinsic__call(execArgv, index - 1, 2);
    }
  }
...
class ChildProcess extends EventEmitter {
  ...
  #handleOnExit(exitCode, signalCode, err) {
    ...
    if (err) {
      ...
      const err2 = new SystemError(`Spawned process exited with error code: ${exitCode}`, __intrinsic__undefined, "spawn", "EUNKNOWN", "ERR_CHILD_PROCESS_UNKNOWN_ERROR");
      err2.pid = this.pid;
      if (this.spawnfile)
        err2.path = this.spawnfile;
      err2.spawnargs = ArrayPrototypeSlice.__intrinsic__call(this.spawnargs, 1);
      ...
      case 0: {
        switch (io) {
          case "pipe": {
            ...
            if (!stdin) {
              const Writable = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 59) || __intrinsic__createInternalModuleById(59);
            ...
            const result = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 23) || __intrinsic__createInternalModuleById(23)).writableFromFileSink(stdin);
      ...
      case 1: {
        switch (io) {
          case "pipe": {
            ...
            if (!value) {
              const Readable = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 55) || __intrinsic__createInternalModuleById(55);
              const stream = new Readable({ read() {} });
              stream.destroy();
              return stream;
            }
            const pipe = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 51) || __intrinsic__createInternalModuleById(51)).constructNativeReadable(value, { encoding });
            this.#closesNeeded++;
  ...
  #createStdioObject() {
    ...
    let result = new __intrinsic__Array(length);
  ...
  spawn(options) {
    validateObject(options, "options");
    validateOneOf(options.serialization, "options.serialization", [__intrinsic__undefined, "json", "advanced"]);
    const serialization = options.serialization || "json";
    const stdio = options.stdio || ["pipe", "pipe", "pipe"];
    const bunStdio = getBunStdioFromOptions(stdio);
    const has_ipc = __intrinsic__isJSArray(stdio) && stdio.includes("ipc");
    if (has_ipc) {
    ...
    this.#encoding = options.encoding || __intrinsic__undefined;
    ...
    try {
      ...
      if (ex != null && typeof ex === "object" && Object.hasOwn(ex, "code") && (ex.code === "EACCES" || ex.code === "EAGAIN" || ex.code === "EMFILE" || ex.code === "ENFILE" || ex.code === "ENOENT")) {
        ...
        ex.spawnargs = __intrinsic__Array.prototype.slice.__intrinsic__call(this.spawnargs, 1);
        process.nextTick(() => {
          this.emit("error", ex);
  ...
  kill(sig) {
    const signal = sig === 0 ? sig : convertToValidSignal(sig === __intrinsic__undefined ? "SIGTERM" : sig);
    const handle = this.#handle;
    if (handle) {
      if (handle.killed) {
        this.killed = true;
...
function nullCheck(path, propName, throwError = true) {
  ...
  const err = __intrinsic__makeErrorWithCode(119, propName, path, "must be a string or Uint8Array without null bytes");
  if (throwError) {
    throw err;
  }
  return err;
...
function ERR_INVALID_OPT_VALUE(name, value) {
  const err = __intrinsic__makeTypeError(`The value "${value}" is invalid for option "${name}"`);
