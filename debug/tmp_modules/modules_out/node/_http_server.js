// @bun
// build/debug/tmp_modules/node/_http_server.ts
var $;
...
var { Duplex, Stream } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117) || __intrinsic__createInternalModuleById(117);
var {
  _checkInvalidHeaderChar: checkInvalidHeaderChar,
  validateHeaderName,
  validateHeaderValue
} = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 73) || __intrinsic__createInternalModuleById(73);
var { validateObject, validateLinkHeaderValue, validateBoolean, validateInteger } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68);
...
var { isPrimary } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 12) || __intrinsic__createInternalModuleById(12);
...
var { format } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 66) || __intrinsic__createInternalModuleById(66);
...
var { kIncomingMessage } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 73) || __intrinsic__createInternalModuleById(73);
var kConnectionsCheckingInterval = Symbol("http.server.connectionsCheckingInterval");
var getBunServerAllClosedPromise = __intrinsic__lazy(40);
...
var kEmptyBuffer = __intrinsic__Buffer.alloc(0);
...
function Server(options, callback) {
  ...
  this[kInternalSocketData] = __intrinsic__undefined;
  this[tlsSymbol] = null;
  this.noDelay = true;
      ...
      onNodeHTTPRequest(bunServer, url, method, headersObject, headersArray, handle, hasBody, socketHandle, isSocketNew, socket, isAncientHTTP, connectHead) {
        ...
        if (method === "CONNECT") {
          if (server.listenerCount("connect") > 0) {
            ...
            const { promise: promise2, resolve: resolve2 } = __intrinsic__newPromiseCapability(__intrinsic__Promise);
        ...
        if (handle.finished || didFinish) {
          ...
          http_res[kCloseCallback] = __intrinsic__undefined;
        ...
        const { resolve, promise } = __intrinsic__newPromiseCapability(__intrinsic__Promise);
...
var NodeHTTPServerSocket = class Socket extends Duplex {
  ...
  [kEnableStreaming](enable) {
    ...
    if (handle) {
      if (enable) {
        ...
        handle.ondrain = __intrinsic__undefined;
      }
    }
  }
  #onDrain() {
  ...
  #onClose() {
    ...
    if (req && !req.complete && !req[kHandle]?.upgraded) {
      req[kHandle] = __intrinsic__undefined;
  ...
  #onCloseForDestroy(closeCallback) {
    this.#onClose();
    if (__intrinsic__isCallable(closeCallback))
      closeCallback();
  }
  _onTimeout() {
    const handle = this[kHandle];
  ...
  _destroy(err, callback) {
    ...
    if (handle.closed) {
      ...
      handle.onclose = __intrinsic__undefined;
...
function ServerResponse(req, options) {
  ...
  if (this[kDeprecatedReplySymbol] = options?.[kDeprecatedReplySymbol]) {
    this[controllerSymbol] = __intrinsic__undefined;
    this[firstWriteSymbol] = __intrinsic__undefined;
    this[deferredSymbol] = __intrinsic__undefined;
  ...
  this.statusCode = 200;
  this.statusMessage = __intrinsic__undefined;
...
`;
  ...
  if (hints.link === null || hints.link === __intrinsic__undefined) {
    return;
  }
...
ServerResponse.prototype.end = function(chunk, encoding, callback) {
  ...
  if (headerState !== NodeHTTPHeaderState.sent) {
    handle.cork(() => {
      ...
      this._contentLength = handle.end(chunk, encoding, __intrinsic__undefined, strictContentLength(this));
    });
  } else {
...
function ensureReadableStreamController(run) {
  ...
  this[runSymbol] = [run];
  this[kDeprecatedReplySymbol](new Response(new __intrinsic__ReadableStream({
    type: "direct",
    pull: (controller) => {
