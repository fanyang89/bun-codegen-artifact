// @bun
// build/debug/tmp_modules/node/dns.ts
var $;
var dns = Bun.dns;
var utilPromisifyCustomSymbol = Symbol.for("nodejs.util.promisify.custom");
...
function translateErrorCode(promise) {
  return promise.catch((error) => {
    return __intrinsic__Promise.__intrinsic__reject(withTranslatedError(error));
  });
}
...
var getRuntimeDefaultResultOrderOption = __intrinsic__lazy(49);
function newResolver(options) {
  if (!newResolver.zig) {
    newResolver.zig = __intrinsic__lazy(50);
  }
...
function setServersOn(servers, object) {
  ...
  servers.forEach((server, i) => {
    ...
    throw __intrinsic__makeErrorWithCode(128, server);
  });
  object.setServers(triples);
}
function validateFlagsOption(options) {
  if (options.flags === __intrinsic__undefined) {
    return;
  }
  validateNumber(options.flags);
  if ((options.flags & ~(dns.ALL | dns.ADDRCONFIG | dns.V4MAPPED)) != 0) {
    throw __intrinsic__makeErrorWithCode(119, "hints", options.flags, "is invalid");
  }
}
function validateFamily(family) {
  if (family !== 6 && family !== 4 && family !== 0) {
    throw __intrinsic__makeErrorWithCode(119, "family", family, "must be one of 0, 4 or 6");
  }
}
...
function invalidHostname(hostname) {
  ...
  process.emitWarning(`The provided hostname "${__intrinsic__String(hostname)}" is not a valid hostname, and is supported in the dns module solely for compatibility.`, "DeprecationWarning", "DEP0118");
}
function translateLookupOptions(options) {
  ...
  if (order === __intrinsic__undefined && typeof verbatim === "boolean") {
...
function lookup(hostname, options, callback) {
  ...
  if (typeof options === "function") {
    ...
    options = { family: options };
  } else if (options !== __intrinsic__undefined && typeof options !== "object") {
    validateFunction(arguments.length === 2 ? options : callback, "callback");
    throw __intrinsic__makeErrorWithCode(118, "options", ["integer", "object"], options);
  }
  validateFunction(callback, "callback");
  ...
  dns.lookup(hostname, options).then((res) => {
    ...
    if (options?.all) {
      ...
      const [{ address, family: family2 }] = res;
      callback(null, address, family2);
    }
  }).catch((err) => {
    if (err.code?.startsWith("DNS_"))
      err.code = err.code.slice(4);
    callback(err, __intrinsic__undefined, __intrinsic__undefined);
  });
}
function lookupService(address, port, callback) {
  if (arguments.length < 3) {
    throw __intrinsic__makeErrorWithCode(150, "address", "port", "callback");
  }
  if (typeof callback !== "function") {
    throw __intrinsic__makeErrorWithCode(118, "callback", "function", callback);
  }
  validateString(address);
  dns.lookupService(address, port).then((results) => {
...
function validateResolverOptions(options) {
  ...
  for (const key of ["timeout", "tries"]) {
    if (key in options) {
      if (typeof options[key] !== "number") {
        throw __intrinsic__makeErrorWithCode(118, key, "number", options[key]);
      }
    }
  }
  if ("timeout" in options) {
    ...
    if (timeout < 0 && timeout != -1 || Math.floor(timeout) != timeout || timeout >= 2 ** 31) {
      throw __intrinsic__makeErrorWithCode(156, "timeout", "Invalid timeout", timeout);
    }
  }
}
var InternalResolver = class Resolver {
  ...
  resolve(hostname, rrtype, callback) {
    if (typeof rrtype === "function") {
      ...
      throw __intrinsic__makeErrorWithCode(118, "rrtype", "string", rrtype);
    }
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolve(hostname, rrtype).then((results) => {
  ...
  resolve4(hostname, options, callback) {
    ...
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolve(hostname, "A").then((addresses) => {
  ...
  resolve6(hostname, options, callback) {
    ...
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolve(hostname, "AAAA").then((addresses) => {
      callback(null, options?.ttl ? addresses : addresses.map(mapResolveX));
    }, (error) => {
      callback(withTranslatedError(error));
    });
  ...
  resolveAny(hostname, callback) {
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolveAny(hostname).then((results) => {
      callback(null, results);
    }, (error) => {
      callback(withTranslatedError(error));
    });
  ...
  resolveCname(hostname, callback) {
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolveCname(hostname).then((results) => {
      callback(null, results);
    }, (error) => {
      callback(withTranslatedError(error));
    });
  ...
  resolveMx(hostname, callback) {
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolveMx(hostname).then((results) => {
      callback(null, results);
    }, (error) => {
      callback(withTranslatedError(error));
    });
  ...
  resolveNaptr(hostname, callback) {
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolveNaptr(hostname).then((results) => {
      callback(null, results);
    }, (error) => {
      callback(withTranslatedError(error));
    });
  ...
  resolveNs(hostname, callback) {
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolveNs(hostname).then((results) => {
      callback(null, results);
    }, (error) => {
      callback(withTranslatedError(error));
    });
  ...
  resolvePtr(hostname, callback) {
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolvePtr(hostname).then((results) => {
      callback(null, results);
    }, (error) => {
      callback(withTranslatedError(error));
    });
  ...
  resolveSrv(hostname, callback) {
    validateResolve(hostname, callback);
    Resolver.#getResolver(this).resolveSrv(hostname).then((results) => {
      callback(null, results);
    }, (error) => {
      callback(withTranslatedError(error));
    });
...
__intrinsic__toClass(Resolver2, "Resolver", InternalResolver);
var {
  resolve,
  resolve4,
  resolve6,
...
var promisifyLookupAll = (order) => (res) => {
  throwIfEmpty(res);
  if (order == "ipv4first") {
    res.sort((a, b) => a.family - b.family);
  } else if (order == "ipv6first") {
...
var promises = {
  ...
  lookup(hostname, options) {
    ...
    if (!hostname) {
      invalidHostname(hostname);
      return __intrinsic__Promise.__intrinsic__resolve(options.all ? [] : {
        address: null,
        family: 4
      });
    }
    ...
    if (family) {
      ...
      return __intrinsic__Promise.__intrinsic__resolve(options.all ? [obj] : obj);
    }
    if (options.all) {
  ...
  lookupService(address, port) {
    if (arguments.length !== 2) {
      throw __intrinsic__makeErrorWithCode(150, "address", "port");
    }
    validateString(address);
    try {
      ...
      return __intrinsic__Promise.__intrinsic__reject(withTranslatedError(err));
    }
  },
  ...
  Resolver: class Resolver3 {
    ...
    resolve(hostname, rrtype) {
      ...
      switch (rrtype?.toLowerCase()) {
        case "a":
        case "aaaa":
          return translateErrorCode(Resolver3.#getResolver(this).resolve(hostname, rrtype).then(promisifyResolveX(false)));
        default:
    ...
    setLocalAddress(first, second) {
      validateLocalAddresses(first, second);
      Resolver3.#getResolver(this).setLocalAddress(first, second);
    }
    setServers(servers) {
