// @bun
// build/debug/tmp_modules/bun/sqlite.ts
var $;
var kSafeIntegersFlag = 1 << 1;
var kStrictFlag = 1 << 2;
var defineProperties = Object.defineProperties;
var toStringTag = Symbol.toStringTag;
var isArray = __intrinsic__Array.isArray;
var isTypedArray = __intrinsic__ArrayBuffer.isView;
var internalFieldTuple;
function initializeSQL() {
  ({ 0: SQL, 1: internalFieldTuple } = __intrinsic__lazy(1));
}
function createChangesObject() {
  return {
    changes: __intrinsic__getInternalField(internalFieldTuple, 0),
    lastInsertRowid: __intrinsic__getInternalField(internalFieldTuple, 1)
  };
}
var constants = {
...
class Statement {
  ...
  *#iterateNoArgs() {
  ...
  safeIntegers(updatedValue) {
    if (updatedValue !== __intrinsic__undefined) {
...
class Database {
  constructor(filenameGiven, options) {
    if (typeof filenameGiven === "undefined") {} else if (typeof filenameGiven !== "string") {
    ...
    if (typeof options === "object" && options) {
      ...
      if ("readOnly" in options)
        __intrinsic__throwTypeError('Misspelled option "readOnly" should be "readonly"');
      if (options.create) {
  ...
  query(query) {
    ...
    while (index !== -1) {
      ...
      const stmt2 = this.#cachedQueriesValues[index];
      if (stmt2.isFinalized) {
        return this.#cachedQueriesValues[index] = this.prepare(query, __intrinsic__undefined, willCache ? constants.SQLITE_PREPARE_PERSISTENT : 0);
      }
      return stmt2;
    }
    var stmt = this.prepare(query, __intrinsic__undefined, willCache ? constants.SQLITE_PREPARE_PERSISTENT : 0);
    if (willCache) {
  ...
  transaction(fn, self) {
    if (typeof fn !== "function")
      __intrinsic__throwTypeError("Expected first argument to be a function");
    const db = this;
...
var getController = (db, _self) => {
  ...
  if (!controller) {
    const shared = {
      commit: db.prepare("COMMIT", __intrinsic__undefined, 0),
      rollback: db.prepare("ROLLBACK", __intrinsic__undefined, 0),
      savepoint: db.prepare("SAVEPOINT `\t_bs3.\t`", __intrinsic__undefined, 0),
      release: db.prepare("RELEASE `\t_bs3.\t`", __intrinsic__undefined, 0),
      rollbackTo: db.prepare("ROLLBACK TO `\t_bs3.\t`", __intrinsic__undefined, 0)
    };
    controllers.set(db, controller = {
      default: Object.assign({ begin: db.prepare("BEGIN", __intrinsic__undefined, 0) }, shared),
      deferred: Object.assign({ begin: db.prepare("BEGIN DEFERRED", __intrinsic__undefined, 0) }, shared),
      immediate: Object.assign({ begin: db.prepare("BEGIN IMMEDIATE", __intrinsic__undefined, 0) }, shared),
      exclusive: Object.assign({ begin: db.prepare("BEGIN EXCLUSIVE", __intrinsic__undefined, 0) }, shared)
    });
  }
  return controller;
};
