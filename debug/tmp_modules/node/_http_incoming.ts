...
// Sourced from src/js/node/_http_incoming.ts
...
const { FakeSocket } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 26/*internal/http/FakeSocket*/) || __intrinsic__createInternalModuleById(26/*internal/http/FakeSocket*/));
...
function assignHeaders(object, req) {
  ...
  if (assignHeadersFast(req, object, headersTuple)) {
    ...
    __intrinsic__putInternalField(headersTuple, 0, undefined);
    __intrinsic__putInternalField(headersTuple, 1, undefined);
...
const IncomingMessagePrototype = {
  ...
  _read(_size) {
    ...
    if (this[noBodySymbol]) {
      ...
      if (completeBody) {
        !(IS_BUN_DEVELOPMENT?$assert(completeBody instanceof ArrayBuffer,"completeBody instanceof ArrayBuffer", "completeBody is not an ArrayBuffer"):void 0);
        !(IS_BUN_DEVELOPMENT?$assert(completeBody.byteLength > 0,"completeBody.byteLength > 0", "completeBody should not be empty"):void 0);
  ...
  _destroy: function IncomingMessage_destroy(err, cb) {
    ...
    if (nodeHTTPResponse) {
      ...
      if (streamState === __intrinsic__streamReadable || streamState === __intrinsic__streamWaiting || streamState === __intrinsic__streamWritable) {
