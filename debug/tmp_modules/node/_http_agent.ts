...
// Sourced from src/js/node/_http_agent.ts
...
const { parseProxyConfigFromEnv, kProxyConfig, checkShouldUseProxy, kWaitForProxyTunnel } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 25/*internal/http*/) || __intrinsic__createInternalModuleById(25/*internal/http*/));
const { getLazy, kEmptyObject, once } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 32/*internal/shared*/) || __intrinsic__createInternalModuleById(32/*internal/shared*/));
const { validateNumber, validateOneOf, validateString } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68/*internal/validators*/) || __intrinsic__createInternalModuleById(68/*internal/validators*/));
const { isIP } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 28/*internal/net/isIP*/) || __intrinsic__createInternalModuleById(28/*internal/net/isIP*/));
...
function freeSocketErrorListener(err) {
  const socket = this;
  (IS_BUN_DEVELOPMENT?$debug_log("SOCKET ERROR on FREE socket:", err.message, err.stack):void 0);
  socket.destroy();
...
function Agent(options): void {
  ...
  if (typeof proxyEnv === "object" && proxyEnv !== null) {
    ...
    (IS_BUN_DEVELOPMENT?$debug_log(`new ${this.protocol} agent with proxy config`, this[kProxyConfig]):void 0);
  }
  ...
  this.on("free", (socket, options) => {
    ...
    (IS_BUN_DEVELOPMENT?$debug_log("agent.on(free)", name):void 0);
...
__intrinsic__toClass(Agent, "Agent", EventEmitter);
...
const tls = getLazy(() => (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 122/*node:tls*/) || __intrinsic__createInternalModuleById(122/*node:tls*/)));
const net = getLazy(() => (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 104/*node:net*/) || __intrinsic__createInternalModuleById(104/*node:net*/)));
...
Agent.prototype.createConnection = function createConnection(...args) {
  ...
  (IS_BUN_DEVELOPMENT?$debug_log(`http createConnection should use proxy for ${options.host}:${options.port}:`, shouldUseProxy):void 0);
  if (!shouldUseProxy) {
    // @ts-ignore
  ...
  !(IS_BUN_DEVELOPMENT?$assert(false,"false", `Unexpected proxy protocol ${proxyProtocol}`):void 0);
};
...
Agent.prototype.addRequest = function addRequest(req, options, port /* legacy */, localAddress /* legacy */) {
  (IS_BUN_DEVELOPMENT?$debug_log("WARN: Agent.addRequest is a no-op"):void 0);
  return; // TODO:
  ...
  if (socket) {
    ...
    this.createSocket(req, options, (err, socket) => {
      if (err) {
        ...
        (IS_BUN_DEVELOPMENT?$debug_log("call onSocket", sockLen, freeLen):void 0);
    ...
    (IS_BUN_DEVELOPMENT?$debug_log("wait for socket"):void 0);
    this.requests[name] ||= [];
...
Agent.prototype.createSocket = function createSocket(req, options, cb) {
  ...
  (IS_BUN_DEVELOPMENT?$debug_log("createConnection", name):void 0);
  ...
  const oncreate = once((err, s) => {
    ...
    (IS_BUN_DEVELOPMENT?$debug_log("sockets", name, this.sockets[name].length, this.totalSocketCount):void 0);
...
function installListeners(agent, s, options) {
  function onFree() {
    (IS_BUN_DEVELOPMENT?$debug_log("CLIENT socket onFree"):void 0);
    agent.emit("free", s, options);
  }
  s.on("free", onFree);

  function onClose() {
    (IS_BUN_DEVELOPMENT?$debug_log("CLIENT socket onClose"):void 0);
  ...
  function onTimeout() {
    (IS_BUN_DEVELOPMENT?$debug_log("CLIENT socket onTimeout"):void 0);
  ...
  function onRemove() {
    (IS_BUN_DEVELOPMENT?$debug_log("CLIENT socket onRemove"):void 0);
    agent.totalSocketCount--;
    agent.removeSocket(s, options);
...
Agent.prototype.removeSocket = function removeSocket(s, options) {
  const name = this.getName(options);
  (IS_BUN_DEVELOPMENT?$debug_log("removeSocket", name, "writable:", s.writable):void 0);
  ...
  if (this.requests[name]?.length) {
    (IS_BUN_DEVELOPMENT?$debug_log("removeSocket, have a request, make a socket"):void 0);
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      (IS_BUN_DEVELOPMENT?$debug_log("removeSocket, have a request with different origin, make a socket"):void 0);
...
Agent.prototype.reuseSocket = function reuseSocket(socket, req) {
  (IS_BUN_DEVELOPMENT?$debug_log("have free socket"):void 0);
