...
// Sourced from src/js/node/zlib.ts


// Hardcoded module "node:zlib"

const BufferModule = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 141/*node:buffer*/) || __intrinsic__createInternalModuleById(141/*node:buffer*/));

const crc32 = __intrinsic__lazy(91);
const NativeZlib = __intrinsic__lazy(92);
const NativeBrotli = __intrinsic__lazy(93);
const NativeZstd = __intrinsic__lazy(94);
...
const kMaxLength = __intrinsic__requireMap.__intrinsic__get("buffer")?.exports.kMaxLength ?? BufferModule.kMaxLength;

const { Transform, finished } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117/*node:stream*/) || __intrinsic__createInternalModuleById(117/*node:stream*/));
const owner_symbol = Symbol("owner_symbol");
const { checkRangesOrGetDefault, validateFunction, validateFiniteNumber } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68/*internal/validators*/) || __intrinsic__createInternalModuleById(68/*internal/validators*/));
...
function zlibBufferOnData(chunk) {
  ...
  else ArrayPrototypePush.__intrinsic__call(this.buffers, chunk);
...
function ZlibBase(opts, mode, handle, { flush, finishFlush, fullFlush }) {
  ...
  !(IS_BUN_DEVELOPMENT?$assert(typeof mode === "number","typeof mode === \"number\""):void 0);
  !(IS_BUN_DEVELOPMENT?$assert(mode >= DEFLATE && mode <= ZSTD_DECOMPRESS,"mode >= DEFLATE && mode <= ZSTD_DECOMPRESS"):void 0);
...
ZlibBase.prototype.reset = function () {
  !(IS_BUN_DEVELOPMENT?$assert(this._handle,"this._handle", "zlib binding closed"):void 0);
...
function processChunkSync(self, chunk, flushFlag) {
  ...
  while (true) {
    ...
    if (have > 0) {
      ...
      !(IS_BUN_DEVELOPMENT?$assert(have === 0,"have === 0", "have should not go down"):void 0);
    }
...
function Brotli(opts, mode) {
  !(IS_BUN_DEVELOPMENT?$assert(mode === BROTLI_DECODE || mode === BROTLI_ENCODE,"mode === BROTLI_DECODE || mode === BROTLI_ENCODE"):void 0);
...
class Zstd extends ZlibBase {
  constructor(opts, mode, initParamsArray, maxParam) {
    !(IS_BUN_DEVELOPMENT?$assert(mode === ZSTD_COMPRESS || mode === ZSTD_DECOMPRESS,"mode === ZSTD_COMPRESS || mode === ZSTD_DECOMPRESS"):void 0);
