...
// Sourced from src/js/node/dns.ts
...
function translateErrorCode(promise: Promise<any>) {
  return promise.catch(error => {
    return Promise.__intrinsic__reject(withTranslatedError(error));
  });
}
...
const getRuntimeDefaultResultOrderOption = __intrinsic__lazy(49);
...
__intrinsic__toClass(Resolver, "Resolver", InternalResolver);
...
const promises = {
  ...
  lookup(hostname, options) {
    ...
    if (family) {
      ...
      return Promise.__intrinsic__resolve(options.all ? [obj] : obj);
    }
  ...
  lookupService(address, port) {
    ...
    try {
      ...
      return Promise.__intrinsic__reject(withTranslatedError(err));
    }
  },
