  ...
  async function runAsyncIterator(controller) {
    ...
    try {
      while (!cancelled && !done) {
        ...
        if (__intrinsic__isPromise(promise) && ($assert(__intrinsic__isPromise(__intrinsic__lazy.temp=(promise))),(__intrinsic__getPromiseInternalField(__intrinsic__lazy.temp, __intrinsic__promiseFieldFlags) & __intrinsic__promiseStateMask) === (__intrinsic__lazy.temp = undefined, __intrinsic__promiseStateFulfilled))) {
