      ...
      if (paramInformation[i] != null) {
        ...
        if (paramGetter[Symbol.asyncIterator] != undefined) {
          for await (const params of paramGetter) {
            ...
            if (__intrinsic__isPromise(result) && ($assert(__intrinsic__isPromise(__intrinsic__lazy.temp=(result))),(__intrinsic__getPromiseInternalField(__intrinsic__lazy.temp, __intrinsic__promiseFieldFlags) & __intrinsic__promiseStateMask) === (__intrinsic__lazy.temp = undefined, __intrinsic__promiseStatePending))) {
              await result;
            }
          }
