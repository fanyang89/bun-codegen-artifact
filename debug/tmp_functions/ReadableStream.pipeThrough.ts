...
$$capture_start$$(function(streams,options) {  const transforms = streams;
  ...
  if (!__intrinsic__isReadableStream(this)) throw __intrinsic__makeErrorWithCode(138, "ReadableStream");

  if (__intrinsic__isReadableStreamLocked(this)) throw __intrinsic__makeErrorWithCode(136, "ReadableStream is locked");

  if (__intrinsic__isWritableStreamLocked(internalWritable)) throw __intrinsic__makeTypeError("WritableStream is locked");

  const promise = __intrinsic__readableStreamPipeToWritableStream(
    this,
    internalWritable,
    preventClose,
    preventAbort,
