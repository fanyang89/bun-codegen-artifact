...
// Sourced from ../../../src/js/builtins/shell.ts

// do not allow the bundler to rename a symbol to $
($);

$$capture_start$$(function(createShellInterpreter_,createParsedShellScript_) {  const createShellInterpreter = createShellInterpreter_ as (
    resolve: (code: number, stdout: Buffer, stderr: Buffer) => void,
    reject: (code: number, stdout: Buffer, stderr: Buffer) => void,
    args: __intrinsic__ZigGeneratedClasses.ParsedShellScript,
  ) => __intrinsic__ZigGeneratedClasses.ShellInterpreter;
  const createParsedShellScript = createParsedShellScript_ as (
    raw: string,
    args: string[],
  ) => __intrinsic__ZigGeneratedClasses.ParsedShellScript;
  ...
  class ShellPromise extends __no_intrinsic__Promise<ShellOutput> {
    #args: __intrinsic__ZigGeneratedClasses.ParsedShellScript | undefined = undefined;
    #hasRun: boolean = false;
    #throws: boolean = true;
    #resolve: (code: number, stdout: Buffer, stderr: Buffer) => void;
    #reject: (code: number, stdout: Buffer, stderr: Buffer) => void;

    constructor(args: __intrinsic__ZigGeneratedClasses.ParsedShellScript, throws: boolean) {
  ...
  class ShellPrototype {
    ...
    env(newEnv: Record<string, string | undefined>) {
      if (typeof newEnv === "undefined" || newEnv === originalDefaultEnv) {
        ...
        __intrinsic__throwTypeError("env must be an object or undefined");
      }
    ...
    cwd(newCwd: string | undefined) {
      if (typeof newCwd === "undefined" || typeof newCwd === "string") {
        ...
        __intrinsic__throwTypeError("cwd must be a string or undefined");
      }
  ...
  function Shell() {
    if (!new.target) {
      __intrinsic__throwTypeError("Class constructor Shell cannot be invoked without 'new'");
    }
