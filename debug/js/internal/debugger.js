...
let $debug_log_enabled = ((env) => (
  ...
  || (env.BUN_DEBUG_DEBUGGER === '1')
  || (env.DEBUG_INTERNAL_DEBUGGER === '1')
))(Bun.env);
let $debug_pid_prefix = Bun.env.SHOW_PID === '1';
let $debug_log = $debug_log_enabled ? (...args) => {
  // warn goes to stderr without colorizing
  console[$debug_trace ? 'trace' : 'warn'](($debug_pid_prefix ? `[${process.pid}] ` : '') + (Bun.enableANSIColors ? '\x1b[90m[internal:debugger]\x1b[0m' : '[internal:debugger]'), ...args);
} : () => {};
...
`);
  ...
  if (notifyUrl) {
    ...
    if (notifyUrl.startsWith("unix://")) {
      const path = @getInternalField(@internalModuleRegistry, 107) || @createInternalModuleById(107);
      notify({
