(function (){"use strict";// build/debug/tmp_modules/internal/cluster/RoundRobinHandle.ts
var $;
var { append, init, isEmpty, peek, remove } = @getInternalField(@internalModuleRegistry, 27) || @createInternalModuleById(27);
...
$ = class RoundRobinHandle {
  ...
  constructor(key, address, { port, fd, flags, backlog, readableAll, writableAll }) {
    net ??= @getInternalField(@internalModuleRegistry, 104) || @createInternalModuleById(104);
    this.key = key;
    this.all = new Map;
    this.free = new Map;
