...
let $assert = function(check, sourceString, ...message) {
  if (!check) {
    ...
    console.error('[internal:sql/mysql] ASSERTION FAILED: ' + sourceString);
...
var { MySQLError } = @getInternalField(@internalModuleRegistry, 33) || @createInternalModuleById(33);
var {
...
class MySQLAdapter {
  ...
  flushConcurrentQueries() {
    ...
    while (true) {
      const nonReservedConnections = @Array.from(this.readyConnections).filter((c) => !(c.flags & 4 /* preReserved */) && c.queryCount < maxDistribution);
