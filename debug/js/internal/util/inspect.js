(function (){"use strict";// build/debug/tmp_modules/internal/util/inspect.ts
var $;
var { pathToFileURL } = @getInternalField(@internalModuleRegistry, 125) || @createInternalModuleById(125);
var BufferModule;
var primordials = @getInternalField(@internalModuleRegistry, 30) || @createInternalModuleById(30);
...
var ArrayPrototypeFlat = uncurryThis(@Array.prototype.flat);
...
var ArrayPrototypeIndexOf = uncurryThis(@Array.prototype.indexOf);
var ArrayPrototypeJoin = uncurryThis(@Array.prototype.join);
var ArrayPrototypeMap = uncurryThis(@Array.prototype.map);
var ArrayPrototypePop = uncurryThis(@Array.prototype.pop);
var ArrayPrototypePush = @Array.prototype.push;
var ArrayPrototypeSlice = uncurryThis(@Array.prototype.slice);
var ArrayPrototypeSplice = uncurryThis(@Array.prototype.splice);
var ArrayPrototypeSort = uncurryThis(@Array.prototype.sort);
var ArrayPrototypeUnshift = uncurryThis(@Array.prototype.unshift);
...
var RegExpPrototypeSymbolReplace = uncurryThis(@RegExp.prototype[Symbol.replace]);
var RegExpPrototypeSymbolSplit = uncurryThis(@RegExp.prototype[Symbol.split]);
var RegExpPrototypeTest = uncurryThis(@RegExp.prototype.test);
var RegExpPrototypeToString = uncurryThis(@RegExp.prototype.toString);
...
var StringPrototypeMatch = uncurryThis(@String.prototype.match);
var StringPrototypeNormalize = uncurryThis(@String.prototype.normalize);
var StringPrototypePadEnd = uncurryThis(@String.prototype.padEnd);
var StringPrototypePadStart = uncurryThis(@String.prototype.padStart);
var StringPrototypeRepeat = uncurryThis(@String.prototype.repeat);
var StringPrototypeReplaceAll = uncurryThis(@String.prototype.replaceAll);
...
var StringPrototypeValueOf = uncurryThis(@String.prototype.valueOf);
...
var extractedSplitNewLinesFastPathStringsOnly = @lazy(19);
...
try {
  ...
  strEscapeSequencesReplacer = new @RegExp("[\x00-\\x1f\\x27\\x5c\\x7f-\\x9f]|[\\ud800-\\udbff](?![\\udc00-\\udfff])|(?<![\\ud800-\\udbff])[\\udc00-\\udfff]", "g");
...
function groupArrayElements(ctx, output, value) {
  ...
  if (actualMax * 3 + ctx.indentationLvl < ctx.breakLength && (totalLength / actualMax > 5 || maxLength <= 6)) {
    ...
    const columns = @min(MathRound(MathSqrt(approxCharHeights * biasedMax * outputLength) / biasedMax), MathFloor((ctx.breakLength - ctx.indentationLvl) / actualMax), ctx.compact * 4, 15);
    if (columns <= 1) {
      return output;
    }
    const tmp = [];
    ...
    for (let i2 = 0;i2 < outputLength; i2 += columns) {
      const max = @min(i2 + columns, outputLength);
      let str = "";
      let j = i2;
      for (;j < max - 1; j++) {
...
function formatArrayBuffer(ctx, value) {
  ...
  let str = StringPrototypeTrim(RegExpPrototypeSymbolReplace(/(.{2})/g, hexSlice(buffer, 0, @min(ctx.maxArrayLength, buffer.length)), "$1 "));
...
function previewEntries(val, isIterator = false) {
  if (isIterator) {
    ...
    const kind = @getInternalField(val, 3);
