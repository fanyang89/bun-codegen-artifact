(function (){"use strict";// build/debug/tmp_modules/internal/streams/writable.ts
var $;
var EE = @getInternalField(@internalModuleRegistry, 96) || @createInternalModuleById(96);
var { Stream } = @getInternalField(@internalModuleRegistry, 50) || @createInternalModuleById(50);
...
function errorBuffer(state) {
  ...
  if ((state[kState] & kBuffered) !== 0) {
    for (let n = state.bufferedIndex;n < state.buffered.length; ++n) {
      ...
      const len = (state[kState] & kObjectMode) !== 0 ? 1 : chunk.length;
      state.length -= len;
      callback(state.errored ?? @makeErrorWithCode(228, "write"));
    }
  }
  callFinishedCallbacks(state, state.errored ?? @makeErrorWithCode(228, "end"));
  resetBuffer(state);
}
