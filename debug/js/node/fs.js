(function (){"use strict";// build/debug/tmp_modules/node/fs.ts
var $;
var EventEmitter = @getInternalField(@internalModuleRegistry, 96) || @createInternalModuleById(96);
var promises = @getInternalField(@internalModuleRegistry, 97) || @createInternalModuleById(97);
var types = @getInternalField(@internalModuleRegistry, 144) || @createInternalModuleById(144);
var { validateFunction, validateInteger } = @getInternalField(@internalModuleRegistry, 68) || @createInternalModuleById(68);
var kEmptyObject = Object.freeze(Object.create(null));
var isDate = types.isDate;
var { fs } = promises.@data;
...
var lchmod = constants.O_SYMLINK !== @undefined ? function lchmod2(path, mode, callback) {
  ensureCallback(callback);
...
var lchmodSync = constants.O_SYMLINK !== @undefined ? fs.lchmodSync.bind(fs) : @undefined;
...
var { defineCustomPromisifyArgs } = @getInternalField(@internalModuleRegistry, 31) || @createInternalModuleById(31);
...
function glob(pattern, options, callback) {
  ...
  validateFunction(callback, "callback");
  @Array.fromAsync(lazyGlob().glob(pattern, options ?? kEmptyObject)).then((result) => callback(null, result)).catch(callback);
}
