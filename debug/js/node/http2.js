(function (){"use strict";// build/debug/tmp_modules/node/http2.ts
var $;
var { isTypedArray } = @getInternalField(@internalModuleRegistry, 144) || @createInternalModuleById(144);
var { hideFromStack, throwNotImplemented } = @getInternalField(@internalModuleRegistry, 32) || @createInternalModuleById(32);
var { STATUS_CODES } = @getInternalField(@internalModuleRegistry, 25) || @createInternalModuleById(25);
var tls = @getInternalField(@internalModuleRegistry, 122) || @createInternalModuleById(122);
...
var { @data } = @getInternalField(@internalModuleRegistry, 97) || @createInternalModuleById(97);
var FileHandle = @data.FileHandle;
...
var Stream = @getInternalField(@internalModuleRegistry, 117) || @createInternalModuleById(117);
var { Readable } = Stream;
var TLSSocket = tls.TLSSocket;
var Socket = net.Socket;
var EventEmitter = @getInternalField(@internalModuleRegistry, 96) || @createInternalModuleById(96);
var { Duplex } = Stream;
var { SafeArrayIterator, SafeSet } = @getInternalField(@internalModuleRegistry, 30) || @createInternalModuleById(30);
...
var H2FrameParser = @lazy(55);
var _nativeAssertSettings = @lazy(56);
var { upgradeRawSocketToH2 } = @getInternalField(@internalModuleRegistry, 70) || @createInternalModuleById(70);
var kSettingIds = {
  1: "headerTableSize",
  2: "enablePush",
...
function throwSettingTypeError(name, value) {
  const err = @makeTypeError(`Invalid value for setting "${name}": ${value}`);
  err.code = "ERR_HTTP2_INVALID_SETTING_VALUE";
  throw err;
}
...
class Http2ServerResponse extends Stream {
  ...
  createPushResponse(headers, callback) {
    ...
    if (this[kState].closed) {
      const error = @makeErrorWithCode(90);
...
function onServerStream(Http2ServerRequest2, Http2ServerResponse2, stream, headers, flags, rawHeaders) {
  const server = this;
  const request = new Http2ServerRequest2(stream, headers, @undefined, rawHeaders);
...
function doSendFileFD(options, fd, headers, err, stat) {
  ...
  if (!stat.isFile()) {
    ...
    if (options.offset !== @undefined || options.offset > 0 || options.length !== @undefined || options.length >= 0 || isDirectory) {
      const err2 = isDirectory ? @makeErrorWithCode(101) : @makeErrorWithCode(100);
      tryClose(fd);
      if (onError)
        onError(err2);
      else {
  ...
  if (this.destroyed || this.closed) {
    tryClose(fd);
    this.destroy(@makeErrorWithCode(90));
    return;
  }
  const statOptions = {
  ...
  if (typeof options.statCheck === "function" && options.statCheck.@call(this, stat, headers, options) === false || this.headersSent) {
    tryClose(fd);
    return;
  }
  if (stat.isFile()) {
...
class ServerHttp2Session extends Http2Session {
  ...
  altsvc(alt, originOrStream) {
    ...
    if (typeof originOrStream === "string") {
      ...
      if (originOrStream >>> 0 !== originOrStream || originOrStream === 0) {
        throw @makeErrorWithCode(156, "originOrStream", `> 0 && < ${2 ** 32}`, originOrStream);
      }
  ...
  ping(payload, callback) {
    ...
    if (typeof callback === "function") {
      if (payload.byteLength !== 8) {
        const error = @makeErrorWithCode(97);
  ...
  destroy(error = NGHTTP2_NO_ERROR, code) {
    ...
    if (typeof error === "number") {
      code = error;
      error = code !== NGHTTP2_NO_ERROR ? @makeErrorWithCode(102, code) : @undefined;
    }
    const socket = this[bunHTTP2Socket];
    if (!this.#connected)
      return;
...
class ClientHttp2Session extends Http2Session {
  ...
  #onConnect() {
    ...
    if (socket instanceof TLSSocket) {
      if (socket.alpnProtocol !== "h2") {
        socket.end();
        const error = @makeErrorWithCode(80, "h2 is not supported");
        this.emit("error", error);
      }
  ...
  #onClose() {
    const parser = this.#parser;
    const err = this.connecting ? @makeErrorWithCode(220) : null;
    if (parser) {
  ...
  request(headers, options) {
    try {
      ...
      if (stream_id < 0) {
        const req2 = new ClientHttp2Stream(@undefined, this, headers);
