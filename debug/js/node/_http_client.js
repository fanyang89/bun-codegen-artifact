...
let $debug_log_enabled = ((env) => (
  ...
  || (env.BUN_DEBUG__HTTP_CLIENT === '1')
  || (env.DEBUG_NODE__HTTP_CLIENT === '1')
))(Bun.env);
let $debug_pid_prefix = Bun.env.SHOW_PID === '1';
let $debug_log = $debug_log_enabled ? (...args) => {
  // warn goes to stderr without colorizing
  console[$debug_trace ? 'trace' : 'warn'](($debug_pid_prefix ? `[${process.pid}] ` : '') + (Bun.enableANSIColors ? '\x1b[90m[_http_client]\x1b[0m' : '[_http_client]'), ...args);
} : () => {};
...
var { validateHeaderName } = @getInternalField(@internalModuleRegistry, 73) || @createInternalModuleById(73);
var { getTimerDuration } = @getInternalField(@internalModuleRegistry, 60) || @createInternalModuleById(60);
...
var { globalAgent } = @getInternalField(@internalModuleRegistry, 71) || @createInternalModuleById(71);
...
function ClientRequest(input, options, cb) {
  ...
  const write_ = (chunk, encoding, callback) => {
    ...
    const canSkipReEncodingData = typeof chunk === "string" && (encoding === "utf-8" || encoding === "utf8" || !encoding) || @isTypedArrayView(chunk) && (!encoding || encoding === "buffer" || encoding === "utf-8");
    let bodySize = 0;
  ...
  const startFetch = (customBody) => {
    ...
    const go = (url, proxy, softFail = false) => {
      const tls = protocol2 === "https:" && this[kTls] ? { ...this[kTls], serverName: this[kTls].servername } : @undefined;
      const fetchOptions = {
        method: method2,
        ...
        handleResponse = () => {
          ...
          process.nextTick((self, res2) => {
            ...
            if (contentLength && isNaN(Number(contentLength))) {
              emitErrorEventNT(self, @makeErrorWithCode(287, "Parse Error"));
              res2.complete = true;
              maybeEmitClose();
              return;
            }
