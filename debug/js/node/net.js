...
let $debug_log_enabled = ((env) => (
  ...
  || (env.BUN_DEBUG_NET === '1')
  || (env.DEBUG_NODE_NET === '1')
))(Bun.env);
let $debug_pid_prefix = Bun.env.SHOW_PID === '1';
let $debug_log = $debug_log_enabled ? (...args) => {
  // warn goes to stderr without colorizing
  console[$debug_trace ? 'trace' : 'warn'](($debug_pid_prefix ? `[${process.pid}] ` : '') + (Bun.enableANSIColors ? '\x1b[90m[net]\x1b[0m' : '[net]'), ...args);
} : () => {};
...
let $assert = function(check, sourceString, ...message) {
  if (!check) {
    ...
    console.error('[net] ASSERTION FAILED: ' + sourceString);
...
var { getDefaultHighWaterMark } = @getInternalField(@internalModuleRegistry, 56) || @createInternalModuleById(56);
...
var { ExceptionWithHostPort, ConnResetException, NodeAggregateError, ErrnoException } = @getInternalField(@internalModuleRegistry, 32) || @createInternalModuleById(32);
var { kTimeout, getTimerDuration } = @getInternalField(@internalModuleRegistry, 60) || @createInternalModuleById(60);
var { validateFunction, validateNumber, validateAbortSignal, validatePort, validateBoolean, validateInt32, validateString } = @getInternalField(@internalModuleRegistry, 68) || @createInternalModuleById(68);
var { isIPv4, isIPv6, isIP } = @getInternalField(@internalModuleRegistry, 28) || @createInternalModuleById(28);
var ArrayPrototypeIncludes = @Array.prototype.includes;
var ArrayPrototypePush = @Array.prototype.push;
var MathMax = Math.max;
var { UV_ECANCELED, UV_ETIMEDOUT } = process.binding("uv");
var isWindows = false;
var getDefaultAutoSelectFamily = @lazy(61);
var setDefaultAutoSelectFamily = @lazy(62);
var getDefaultAutoSelectFamilyAttemptTimeout = @lazy(63);
var setDefaultAutoSelectFamilyAttemptTimeout = @lazy(64);
var SocketAddress = @lazy(65);
var BlockList = @lazy(66);
var newDetachedSocket = @lazy(67);
var doConnect = @lazy(68);
var addServerName = @lazy(69);
var upgradeDuplexToTLS = @lazy(38);
var isNamedPipeSocket = @lazy(70);
var getBufferedAmount = @lazy(71);
...
var SocketHandlers2 = {
  open(socket) {
    ...
    $debug_log("self[kupgraded]", @String(self[kupgraded]));
    if (!self[kupgraded])
...
Socket.prototype.connect = function connect(...args) {
  ...
  {
    const [options2, connectListener] = @isArray(args[0]) && args[0][normalizedArgsSymbol] ? args[0] : normalizeArgs(args);
...
Socket.prototype._write = function _write(chunk, encoding, callback) {
  ...
  const socket = this._handle;
  if (!socket) {
    callback(@makeErrorWithCode(220));
    return false;
  }
  this._unrefTimer();
...
function lookupAndConnect(self, options) {
  ...
  lookup(host, dnsopts, function emitLookup(err, ip, addressType2) {
    ...
    if (err) {
      ...
      err = @makeErrorWithCode(128, ip);
      process.nextTick(destroyNT, self, err);
    } else if (addressType2 !== 4 && addressType2 !== 6) {
      err = @makeErrorWithCode(117, addressType2, options.host, options.port);
      process.nextTick(destroyNT, self, err);
    } else {
      self._unrefTimer();
...
function lookupAndConnectMultiple(self, lookup, host, options, dnsopts, port, localAddress, localPort, timeout) {
  lookup(host, dnsopts, function emitLookup(err, addresses) {
    ...
    for (let i = 0, l = addresses.length;i < l; i++) {
      ...
      if (isIP(ip) && (addressType === 4 || addressType === 6)) {
        ...
        if (!ArrayPrototypeIncludes.@call(validIps[destination], ip)) {
          ArrayPrototypePush.@call(validAddresses[destination], address2);
    ...
    if (!validAddresses[0].length && !validAddresses[1].length) {
      ...
      if (!isIP(firstIp)) {
        err = @makeErrorWithCode(128, firstIp);
        process.nextTick(destroyNT, self, err);
      } else if (firstAddressType !== 4 && firstAddressType !== 6) {
        err = @makeErrorWithCode(117, firstAddressType, options.host, options.port);
        process.nextTick(destroyNT, self, err);
      }
      return;
    }
...
function internalConnectMultiple(context, canceled) {
  ...
  if (self.blockList?.check(address2, `ipv${addressType}`)) {
    const ex = @makeErrorWithCode(142, address2);
  ...
  req.addressType = addressType;
  req.tls = tls;
  ArrayPrototypePush.@call(self.autoSelectFamilyAttemptedAddresses, `${address2}:${port}`);
  err = kConnectTcp(self, addressType, req, address2, port);
  if (err) {
    const ex = new ExceptionWithHostPort(err, "connect", address2, port);
    ArrayPrototypePush.@call(context.errors, ex);
