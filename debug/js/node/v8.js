(function (){"use strict";// build/debug/tmp_modules/node/v8.ts
var $;
var { hideFromStack, throwNotImplemented } = @getInternalField(@internalModuleRegistry, 32) || @createInternalModuleById(32);
var jsc = @getInternalField(@internalModuleRegistry, 139) || @createInternalModuleById(139);
...
function getHeapSnapshot() {
  if (!HeapSnapshotReadable_) {
    const Readable = (@getInternalField(@internalModuleRegistry, 117) || @createInternalModuleById(117)).Readable;
...
function getDefaultHeapSnapshotPath() {
  const date = new Date;
  const worker_threads = @getInternalField(@internalModuleRegistry, 130) || @createInternalModuleById(130);
