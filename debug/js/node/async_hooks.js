...
let $debug_log_enabled = ((env) => (
  ...
  || (env.BUN_DEBUG_ASYNC_HOOKS === '1')
  || (env.DEBUG_NODE_ASYNC_HOOKS === '1')
))(Bun.env);
let $debug_pid_prefix = Bun.env.SHOW_PID === '1';
let $debug_log = $debug_log_enabled ? (...args) => {
  // warn goes to stderr without colorizing
  console[$debug_trace ? 'trace' : 'warn'](($debug_pid_prefix ? `[${process.pid}] ` : '') + (Bun.enableANSIColors ? '\x1b[90m[async_hooks]\x1b[0m' : '[async_hooks]'), ...args);
} : () => {};
...
var $;
var setAsyncHooksEnabled = @lazy(41);
var cleanupLater = @lazy(42);
var { validateFunction, validateString, validateObject } = @getInternalField(@internalModuleRegistry, 68) || @createInternalModuleById(68);
...
class AsyncLocalStorage {
  ...
  constructor() {
    ...
    if (true) {
      ...
      const source = (@getInternalField(@internalModuleRegistry, 139) || @createInternalModuleById(139)).callerSourceOrigin();
      this.__id__ = uid + "@" + (@getInternalField(@internalModuleRegistry, 107) || @createInternalModuleById(107)).basename(source);
