    ...
    for (let [filter, callback] of results) {
      if (filter.test(inputPath)) {
        var result = callback({
          ...
          resolveDir: inputNamespace === "file" ? (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 107/*node:path*/) || __intrinsic__createInternalModuleById(107/*node:path*/)).dirname(importer) : undefined,
          kind,
          // pluginData
        });
        ...
        if (path !== undefined && typeof path !== "string") {
          __intrinsic__throwTypeError("onResolve plugins 'path' field must be a string if provided");
        }

        if (result.namespace !== undefined && typeof result.namespace !== "string") {
          __intrinsic__throwTypeError("onResolve plugins 'namespace' field must be a string if provided");
        }
        ...
        if (!external) {
          if (userNamespace === "file") {
            if (process.platform !== "win32") {
              ...
              if ((__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 107/*node:path*/) || __intrinsic__createInternalModuleById(107/*node:path*/)).isAbsolute(path) === false || path.includes("..")) {
                __intrinsic__throwTypeError('onResolve plugin "path" must be absolute when the namespace is "file"');
              }
            }
          }
