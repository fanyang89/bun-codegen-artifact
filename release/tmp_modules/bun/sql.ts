...
// Sourced from src/js/bun/sql.ts
...
const { Query, SQLQueryFlags } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 36/*internal/sql/query*/) || __intrinsic__createInternalModuleById(36/*internal/sql/query*/));
const { PostgresAdapter } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 35/*internal/sql/postgres*/) || __intrinsic__createInternalModuleById(35/*internal/sql/postgres*/));
const { MySQLAdapter } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 34/*internal/sql/mysql*/) || __intrinsic__createInternalModuleById(34/*internal/sql/mysql*/));
const { SQLiteAdapter } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 38/*internal/sql/sqlite*/) || __intrinsic__createInternalModuleById(38/*internal/sql/sqlite*/));
const { SQLHelper, parseOptions } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 37/*internal/sql/shared*/) || __intrinsic__createInternalModuleById(37/*internal/sql/shared*/));

const { SQLError, PostgresError, SQLiteError, MySQLError } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 33/*internal/sql/errors*/) || __intrinsic__createInternalModuleById(33/*internal/sql/errors*/));
...
const SQL: typeof Bun.SQL = function SQL(
  ...
  function queryFromPool(
    ...
    try {
      ...
      return Promise.__intrinsic__reject(err);
    }
  }
  ...
  function onReserveConnected(this: Query<any, any>, err: Error | null, pooledConnection) {
    ...
    function reserved_sql(strings: string | TemplateStringsArray | SQLHelper<any> | Query<any, any>, ...values: any[]) {
      if (
        ...
        return Promise.__intrinsic__reject(pool.connectionClosedError());
      }
    ...
    reserved_sql.connect = () => {
      ...
      return Promise.__intrinsic__resolve(reserved_sql);
    };
    ...
    reserved_sql.beginDistributed = (name: string, fn: TransactionCallback) => {
      ...
      if (typeof name !== "string") {
        return Promise.__intrinsic__reject(__intrinsic__makeErrorWithCode(119, "name", name, "must be a string"));
      }

      if (!__intrinsic__isCallable(callback)) {
        return Promise.__intrinsic__reject(__intrinsic__makeErrorWithCode(119, "fn", callback, "must be a function"));
      }
    ...
    reserved_sql.begin = (options_or_fn: string | TransactionCallback, fn?: TransactionCallback) => {
      ...
      if (__intrinsic__isCallable(options_or_fn)) {
        ...
        return Promise.__intrinsic__reject(__intrinsic__makeErrorWithCode(119, "options", options_or_fn, "must be a string"));
      }
      if (!__intrinsic__isCallable(callback)) {
        return Promise.__intrinsic__reject(__intrinsic__makeErrorWithCode(119, "fn", callback, "must be a function"));
      }
    ...
    reserved_sql.close = async (options?: { timeout?: number }) => {
      ...
      if (
        ...
        return Promise.__intrinsic__resolve(undefined);
      }
      state.connectionState &= ~ReservedConnectionState.acceptQueries;
  ...
  async function onTransactionConnected(
    ...
    */
    ...
    function transaction_sql(
      ...
      if (__intrinsic__isArray(strings)) {
        // detect if is tagged template
        if (!__intrinsic__isArray((strings as unknown as TemplateStringsArray).raw)) {
          return new SQLHelper(strings, values);
        }
      } else if (typeof strings === "object" && !(strings instanceof Query) && !(strings instanceof SQLHelper)) {
        return new SQLHelper([strings], values);
    ...
    transaction_sql.connect = () => {
      ...
      return Promise.__intrinsic__resolve(transaction_sql);
    };
  ...
  sql.reserve = () => {
    ...
    if (pool.supportsReservedConnections && !pool.supportsReservedConnections()) {
      return Promise.__intrinsic__reject(new Error("This adapter doesn't support connection reservation"));
    }
  ...
  sql.connect = () => {
    ...
    if (pool.isConnected()) {
      return Promise.__intrinsic__resolve(sql);
    }
