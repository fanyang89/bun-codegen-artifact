...
class ExceptionWithHostPort extends Error {
  ...
  constructor(err: number, syscall: string, address: string, port?: number) {
    ...
    util ??= (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 126/*node:util*/) || __intrinsic__createInternalModuleById(126/*node:util*/));
...
function once(callback, { preserveReturnValue = false } = kEmptyObject) {
  ...
  return function (...args) {
    ...
    called = true;
    const result = callback.__intrinsic__apply(this, args);
