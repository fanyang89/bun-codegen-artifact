...
// Sourced from src/js/internal/streams/readable.ts


"use strict";

const EE = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 96/*node:events*/) || __intrinsic__createInternalModuleById(96/*node:events*/));
const { Stream, prependListener } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 50/*internal/streams/legacy*/) || __intrinsic__createInternalModuleById(50/*internal/streams/legacy*/));
...
const { getHighWaterMark, getDefaultHighWaterMark } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 56/*internal/streams/state*/) || __intrinsic__createInternalModuleById(56/*internal/streams/state*/));
const {
  kOnConstructed,
  kState,
  // bitfields
  ...
  kErrored,
  kConstructed,
} = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 58/*internal/streams/utils*/) || __intrinsic__createInternalModuleById(58/*internal/streams/utils*/));
const { aggregateTwoErrors } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 16/*internal/errors*/) || __intrinsic__createInternalModuleById(16/*internal/errors*/));
const { validateObject } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68/*internal/validators*/) || __intrinsic__createInternalModuleById(68/*internal/validators*/));
const { StringDecoder } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 143/*node:string_decoder*/) || __intrinsic__createInternalModuleById(143/*node:string_decoder*/));
const from = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 48/*internal/streams/from*/) || __intrinsic__createInternalModuleById(48/*internal/streams/from*/));
const { SafeSet } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 30/*internal/primordials*/) || __intrinsic__createInternalModuleById(30/*internal/primordials*/));
const { kAutoDestroyed } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 32/*internal/shared*/) || __intrinsic__createInternalModuleById(32/*internal/shared*/));
...
__intrinsic__toClass(Readable, "Readable", Stream);
...
Readable.prototype[SymbolAsyncDispose] = function () {
  let error;
  if (!this.destroyed) {
    error = this.readableEnded ? null : __intrinsic__makeAbortError();
    this.destroy(error);
  }
...
Readable.prototype.push = function (chunk, encoding) {
  (IS_BUN_DEVELOPMENT?$debug_log("push", chunk):void 0);
...
Readable.prototype.unshift = function (chunk, encoding) {
  (IS_BUN_DEVELOPMENT?$debug_log("unshift", chunk):void 0);
  const state = this._readableState;
  return (state[kState] & kObjectMode) === 0
...
function readableAddChunkUnshiftByteMode(stream, state, chunk, encoding) {
  ...
  if (typeof chunk === "string") {
    ...
    errorOrDestroy(stream, __intrinsic__makeErrorWithCode(118, "chunk", ["string", "Buffer", "TypedArray", "DataView"], chunk));
    return false;
  }
...
function readableAddChunkUnshiftValue(stream, state, chunk) {
  if ((state[kState] & kEndEmitted) !== 0) errorOrDestroy(stream, __intrinsic__makeErrorWithCode(234, ));
...
function readableAddChunkPushByteMode(stream, state, chunk, encoding) {
  ...
  if ((state[kState] & kEnded) !== 0) {
    errorOrDestroy(stream, __intrinsic__makeErrorWithCode(231, ));
    return false;
  }
...
function computeNewHighWaterMark(n) {
  if (n > MAX_HWM) {
    throw __intrinsic__makeErrorWithCode(156, "size", "<= 1GiB", n);
  } else {
...
Readable.prototype.read = function (n) {
  (IS_BUN_DEVELOPMENT?$debug_log("read", n):void 0);
  ...
  if (
    ...
    (IS_BUN_DEVELOPMENT?$debug_log("read: emitReadable"):void 0);
  ...
  (IS_BUN_DEVELOPMENT?$debug_log("need readable", doRead):void 0);

  // If we currently have less than the highWaterMark, then also read some.
  if (state.length === 0 || state.length - n < state.highWaterMark) {
    doRead = true;
    (IS_BUN_DEVELOPMENT?$debug_log("length less than watermark", doRead):void 0);
  }
  ...
  if ((state[kState] & (kReading | kEnded | kDestroyed | kErrored | kConstructed)) !== kConstructed) {
    doRead = false;
    (IS_BUN_DEVELOPMENT?$debug_log("reading, ended or constructing", doRead):void 0);
  } else if (doRead) {
    (IS_BUN_DEVELOPMENT?$debug_log("do read"):void 0);
...
function onEofChunk(stream, state) {
  (IS_BUN_DEVELOPMENT?$debug_log("onEofChunk"):void 0);
...
function emitReadable(stream) {
  ...
  (IS_BUN_DEVELOPMENT?$debug_log("emitReadable"):void 0);
  state[kState] &= ~kNeedReadable;
  if ((state[kState] & kEmittedReadable) === 0) {
    (IS_BUN_DEVELOPMENT?$debug_log("emitReadable", (state[kState] & kFlowing) !== 0):void 0);
    state[kState] |= kEmittedReadable;
...
function emitReadable_(stream) {
  const state = stream._readableState;
  (IS_BUN_DEVELOPMENT?$debug_log("emitReadable_"):void 0);
...
function maybeReadMore_(stream, state) {
  ...
  while (
    ...
    (IS_BUN_DEVELOPMENT?$debug_log("maybeReadMore read 0"):void 0);
    stream.read(0);
    if (len === state.length)
...
Readable.prototype._read = function (_n) {
  throw __intrinsic__makeErrorWithCode(149, "_read()");
};
...
Readable.prototype.pipe = function (dest, pipeOpts) {
  ...
  (IS_BUN_DEVELOPMENT?$debug_log("pipe count=%d opts=%j", state.pipes.length, pipeOpts):void 0);
  ...
  function onunpipe(readable, unpipeInfo) {
    (IS_BUN_DEVELOPMENT?$debug_log("onunpipe"):void 0);
    if (readable === src) {
  ...
  function onend() {
    (IS_BUN_DEVELOPMENT?$debug_log("onend"):void 0);
    dest.end();
  }
  ...
  let cleanedUp = false;
  function cleanup() {
    (IS_BUN_DEVELOPMENT?$debug_log("cleanup"):void 0);
  ...
  function pause() {
    ...
    if (!cleanedUp) {
      if (state.pipes.length === 1 && state.pipes[0] === dest) {
        (IS_BUN_DEVELOPMENT?$debug_log("false write response, pause", 0):void 0);
        state.awaitDrainWriters = dest;
        state[kState] &= ~kMultiAwaitDrain;
      } else if (state.pipes.length > 1 && state.pipes.includes(dest)) {
        (IS_BUN_DEVELOPMENT?$debug_log("false write response, pause", state.awaitDrainWriters.size):void 0);
  ...
  function ondata(chunk) {
    (IS_BUN_DEVELOPMENT?$debug_log("ondata"):void 0);
    try {
      const ret = dest.write(chunk);
      (IS_BUN_DEVELOPMENT?$debug_log("dest.write", ret):void 0);
      if (ret === false) {
        pause();
      }
    } catch (err) {
...
Readable.prototype.unpipe = function (dest) {
  ...
  const index = ArrayPrototypeIndexOf.__intrinsic__call(state.pipes, dest);
...
Readable.prototype.on = function (ev, fn) {
  ...
  if (ev === "data") {
    ...
    if ((state[kState] & (kEndEmitted | kReadableListening)) === 0) {
      ...
      (IS_BUN_DEVELOPMENT?$debug_log("on readable"):void 0);
      if (state.length) {
        emitReadable(this);
...
function nReadingNextTick(self) {
  (IS_BUN_DEVELOPMENT?$debug_log("readable nexttick read 0"):void 0);
  self.read(0);
}
...
Readable.prototype.resume = function () {
  ...
  if ((state[kState] & kFlowing) === 0) {
    (IS_BUN_DEVELOPMENT?$debug_log("resume"):void 0);
...
function resume_(stream, state) {
  (IS_BUN_DEVELOPMENT?$debug_log("resume", (state[kState] & kReading) !== 0):void 0);
...
Readable.prototype.pause = function () {
  const state = this._readableState;
  (IS_BUN_DEVELOPMENT?$debug_log("call pause"):void 0);
  if ((state[kState] & (kHasFlowing | kFlowing)) !== kHasFlowing) {
    (IS_BUN_DEVELOPMENT?$debug_log("pause"):void 0);
    state[kState] |= kHasFlowing;
    state[kState] &= ~kFlowing;
    this.emit("pause");
  }
...
function flow(stream) {
  const state = stream._readableState;
  (IS_BUN_DEVELOPMENT?$debug_log("flow"):void 0);
...
function fromList(n, state) {
  ...
  if ((state[kState] & kObjectMode) !== 0) {
    ...
    if ((state[kState] & kDecoder) !== 0) {
      ...
      let i = 0;
      while (idx < len) {
        TypedArrayPrototypeSet.__intrinsic__call(ret, buf[idx], i);
        i += buf[idx].length;
        buf[idx++] = null;
      }
    }
    ...
    while (idx < len) {
      const data = buf[idx];
      if (n > data.length) {
        TypedArrayPrototypeSet.__intrinsic__call(ret, data, retLen - n);
        n -= data.length;
        buf[idx++] = null;
      } else {
        if (n === data.length) {
          ...
          TypedArrayPrototypeSet.__intrinsic__call(ret, new __intrinsic__Buffer(data.buffer, data.byteOffset, n), retLen - n);
          buf[idx] = new __intrinsic__Buffer(data.buffer, data.byteOffset + n, data.length - n);
        }
        break;
      }
    }
...
function endReadable(stream) {
  ...
  (IS_BUN_DEVELOPMENT?$debug_log("endReadable"):void 0);
...
function endReadableNT(state, stream) {
  (IS_BUN_DEVELOPMENT?$debug_log("endReadableNT"):void 0);
...
$ = Readable as unknown as typeof import("node:stream").Readable;
;$$EXPORT$$($).$$EXPORT_END$$;
