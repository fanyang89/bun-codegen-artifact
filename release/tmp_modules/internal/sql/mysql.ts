...
// Sourced from src/js/internal/sql/mysql.ts
...
;
;
const { SQLHelper, SSLMode, SQLResultArray, buildDefinedColumnsAndQuery } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 37/*internal/sql/shared*/) || __intrinsic__createInternalModuleById(37/*internal/sql/shared*/));
const {
  Query,
  SQLQueryFlags,
  symbols: { _strings, _values, _flags, _results, _handle },
} = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 36/*internal/sql/query*/) || __intrinsic__createInternalModuleById(36/*internal/sql/query*/));
const { MySQLError } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 33/*internal/sql/errors*/) || __intrinsic__createInternalModuleById(33/*internal/sql/errors*/));

const {
  createConnection: createMySQLConnection,
  createQuery: createMySQLQuery,
  init: initMySQL,
} = __intrinsic__lazy(16) as MySQLDotZig;
...
initMySQL(
  function onResolveMySQLQuery(query, result, commandTag, count, queries, is_last, last_insert_rowid, affected_rows) {
    ...
    if (query[_flags] & SQLQueryFlags.simple) {
      !(IS_BUN_DEVELOPMENT?$assert(result instanceof SQLResultArray,"result instanceof SQLResultArray", "Invalid result array"):void 0);
...
export interface MySQLDotZig {
  ...
  createConnection: (
    ...
    query: string,
    path: string,
    onConnected: (err: Error | null, connection: __intrinsic__ZigGeneratedClasses.MySQLConnection) => void,
    onDisconnected: (err: Error | null, connection: __intrinsic__ZigGeneratedClasses.MySQLConnection) => void,
    idleTimeout: number,
    connectionTimeout: number,
    maxLifetime: number,
    useUnnamedPreparedStatements: boolean,
  ) => __intrinsic__ZigGeneratedClasses.MySQLConnection;
  createQuery: (
    sql: string,
    values: unknown[],
    ...
    bigint: boolean,
    simple: boolean,
  ) => __intrinsic__ZigGeneratedClasses.MySQLQuery;
}
...
class PooledMySQLConnection {
  ...
  ): Promise<__intrinsic__ZigGeneratedClasses.MySQLConnection | null> {
    const {
      hostname,
      port,
      username,
  ...
  connection: __intrinsic__ZigGeneratedClasses.MySQLConnection | null = null;
...
class MySQLAdapter
  implements
    DatabaseAdapter<PooledMySQLConnection, __intrinsic__ZigGeneratedClasses.MySQLConnection, __intrinsic__ZigGeneratedClasses.MySQLQuery>
{
