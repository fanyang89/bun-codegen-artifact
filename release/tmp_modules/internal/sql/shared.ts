...
// Sourced from src/js/internal/sql/shared.ts
...
class SQLHelper<T> {
  ...
  constructor(value: T, keys?: (keyof T)[]) {
    if (keys !== undefined && keys.length === 0 && (__intrinsic__isObject(value[0]) || __intrinsic__isArray(value[0]))) {
...
function parseOptions(
  ...
  if (adapter === "postgres") {
    ...
    if (path && Number.isSafeInteger(port) && path?.indexOf("/.s.PGSQL.") === -1) {
      ...
      if ((__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 98/*node:fs*/) || __intrinsic__createInternalModuleById(98/*node:fs*/)).existsSync(pathWithSocket)) {
        path = pathWithSocket;
      }
    }
  }
  ...
  if (options.prepare === false) {
    if (adapter === "mysql") {
      throw __intrinsic__makeErrorWithCode(119, "options.prepare", false, "prepared: false is not supported in MySQL");
    }
    prepare = false;
  }
  ...
  if (path) {
    if ((__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 98/*node:fs*/) || __intrinsic__createInternalModuleById(98/*node:fs*/)).existsSync(path)) {
      ret.path = path;
    }
  }
