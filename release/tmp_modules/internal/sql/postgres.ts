...
// Sourced from src/js/internal/sql/postgres.ts
...
} = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 37/*internal/sql/shared*/) || __intrinsic__createInternalModuleById(37/*internal/sql/shared*/));
const {
  Query,
  SQLQueryFlags,
...
const { PostgresError } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 33/*internal/sql/errors*/) || __intrinsic__createInternalModuleById(33/*internal/sql/errors*/));

const {
  createConnection: createPostgresConnection,
  createQuery: createPostgresQuery,
  init: initPostgres,
} = __intrinsic__lazy(17) as PostgresDotZig;
...
function serializeArray(values: any[], type: ArrayType) {
  if (!__intrinsic__isArray(values) && !isTypedArray(values)) return values;
...
export interface PostgresDotZig {
  ...
  createConnection: (
    ...
    query: string,
    path: string,
    onConnected: (err: Error | null, connection: __intrinsic__ZigGeneratedClasses.PostgresSQLConnection) => void,
    onDisconnected: (err: Error | null, connection: __intrinsic__ZigGeneratedClasses.PostgresSQLConnection) => void,
    idleTimeout: number,
    connectionTimeout: number,
    maxLifetime: number,
    useUnnamedPreparedStatements: boolean,
  ) => __intrinsic__ZigGeneratedClasses.PostgresSQLConnection;
  createQuery: (
    sql: string,
    values: unknown[],
    ...
    bigint: boolean,
    simple: boolean,
  ) => __intrinsic__ZigGeneratedClasses.PostgresSQLQuery;
}
...
class PooledPostgresConnection {
  ...
  ): Promise<__intrinsic__ZigGeneratedClasses.PostgresSQLConnection | null> {
    const {
      hostname,
      port,
      username,
  ...
  connection: __intrinsic__ZigGeneratedClasses.PostgresSQLConnection | null = null;
