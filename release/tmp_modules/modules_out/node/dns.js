// @bun
// build/release/tmp_modules/node/dns.ts
var $, dns = Bun.dns, utilPromisifyCustomSymbol = Symbol.for("nodejs.util.promisify.custom"), { isIP } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 28) || __intrinsic__createInternalModuleById(28), {
  validateFunction,
  validateArray,
  validateString,
  validateBoolean,
  validateNumber
} = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), errorCodes = {
  NODATA: "ENODATA",
  FORMERR: "EFORMERR",
  SERVFAIL: "ESERVFAIL",
  NOTFOUND: "ENOTFOUND",
...
function invalidHostname(hostname) {
  if (invalidHostname.warned)
    return;
  invalidHostname.warned = !0, process.emitWarning(`The provided hostname "${__intrinsic__String(hostname)}" is not a valid hostname, and is supported in the dns module solely for compatibility.`, "DeprecationWarning", "DEP0118");
}
...
function lookup(hostname, options, callback) {
  ...
  if (typeof options === "function")
    ...
    throw validateFunction(arguments.length === 2 ? options : callback, "callback"), __intrinsic__makeErrorWithCode(118, "options", ["integer", "object"], options);
...
var promisifyLookup = (order) => (res) => {
  ...
  lookup(hostname, options) {
    ...
    if (options = translateLookupOptions(options), validateLookupOptions(options), !hostname)
      return invalidHostname(hostname), __intrinsic__Promise.__intrinsic__resolve(options.all ? [] : {
        address: null,
        family: 4
      });
    let family = isIP(hostname);
