// @bun
// build/release/tmp_modules/node/net.ts
var $, Duplex = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 44) || __intrinsic__createInternalModuleById(44), { getDefaultHighWaterMark } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 56) || __intrinsic__createInternalModuleById(56), EventEmitter = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 96) || __intrinsic__createInternalModuleById(96), dns, normalizedArgsSymbol = Symbol("normalizedArgs"), { ExceptionWithHostPort, ConnResetException, NodeAggregateError, ErrnoException } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 32) || __intrinsic__createInternalModuleById(32), { kTimeout, getTimerDuration } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 60) || __intrinsic__createInternalModuleById(60), { validateFunction, validateNumber, validateAbortSignal, validatePort, validateBoolean, validateInt32, validateString } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), { isIPv4, isIPv6, isIP } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 28) || __intrinsic__createInternalModuleById(28), ArrayPrototypeIncludes = __intrinsic__Array.prototype.includes, ArrayPrototypePush = __intrinsic__Array.prototype.push, MathMax = Math.max, { UV_ECANCELED, UV_ETIMEDOUT } = process.binding("uv");
var getDefaultAutoSelectFamily = __intrinsic__lazy(61), setDefaultAutoSelectFamily = __intrinsic__lazy(62), getDefaultAutoSelectFamilyAttemptTimeout = __intrinsic__lazy(63), setDefaultAutoSelectFamilyAttemptTimeout = __intrinsic__lazy(64), SocketAddress = __intrinsic__lazy(65), BlockList = __intrinsic__lazy(66), newDetachedSocket = __intrinsic__lazy(67), doConnect = __intrinsic__lazy(68), addServerName = __intrinsic__lazy(69), upgradeDuplexToTLS = __intrinsic__lazy(38), isNamedPipeSocket = __intrinsic__lazy(70), getBufferedAmount = __intrinsic__lazy(71), bunTlsSymbol = Symbol.for("::buntls::"), bunSocketServerOptions = Symbol.for("::bunnetserveroptions::"), owner_symbol = Symbol("owner_symbol"), kServerSocket = Symbol("kServerSocket"), kBytesWritten = Symbol("kBytesWritten"), bunTLSConnectOptions = Symbol.for("::buntlsconnectoptions::"), kReinitializeHandle = Symbol("kReinitializeHandle"), kRealListen = Symbol("kRealListen"), kSetNoDelay = Symbol("kSetNoDelay"), kSetKeepAlive = Symbol("kSetKeepAlive"), kSetKeepAliveInitialDelay = Symbol("kSetKeepAliveInitialDelay"), kConnectOptions = Symbol("connect-options"), kAttach = Symbol("kAttach"), kCloseRawConnection = Symbol("kCloseRawConnection"), kpendingRead = Symbol("kpendingRead"), kupgraded = Symbol("kupgraded"), ksocket = Symbol("ksocket"), khandlers = Symbol("khandlers"), kclosed = Symbol("closed"), kended = Symbol("ended"), kwriteCallback = Symbol("writeCallback"), kSocketClass = Symbol("kSocketClass");
function endNT(socket, callback, err) {
  socket.__intrinsic__end(), callback(err);
}
...
var ServerHandlers = {
  ...
  open(socket) {
    ...
    if (socket.data.req = __intrinsic__undefined, self.pauseOnConnect)
      self.pause();
    if (self[kupgraded]) {
      self.connecting = !1;
      let options = self[bunTLSConnectOptions];
  ...
  connectError(socket, error) {
    let { self, req } = socket.data;
    socket[owner_symbol] = self, req.oncomplete(error.errno, self._handle, req, !0, !0), socket.data.req = __intrinsic__undefined;
  }
};
...
function Socket(options) {
  ...
  if (Duplex.__intrinsic__call(this, {
    ...opts,
    allowHalfOpen,
    readable: !0,
    writable: !0,
    ...
    autoDestroy: !0,
    decodeStrings: !1
  }), this._parent = null, this._parentWrap = null, this[kpendingRead] = __intrinsic__undefined, this[kupgraded] = null, this[kSetNoDelay] = Boolean(noDelay), this[kSetKeepAlive] = Boolean(keepAlive), this[kSetKeepAliveInitialDelay] = ~~(keepAliveInitialDelay / 1000), this[khandlers] = SocketHandlers2, this.bytesRead = 0, this[kBytesWritten] = __intrinsic__undefined, this[kclosed] = !1, this[kended] = !1, this.connecting = !1, this._host = __intrinsic__undefined, this._port = __intrinsic__undefined, this[bunTLSConnectOptions] = null, this.timeout = 0, this[kwriteCallback] = __intrinsic__undefined, this._pendingData = __intrinsic__undefined, this._pendingEncoding = __intrinsic__undefined, this._hadError = !1, this.isServer = !1, this._handle = null, this[ksocket] = __intrinsic__undefined, this.server = __intrinsic__undefined, this.pauseOnConnect = !1, this._peername = null, this._sockname = null, this._closeAfterHandlingError = !1, this.on("end", onSocketEnd), options?.fd !== __intrinsic__undefined) {
    let { fd } = options;
...
Socket.prototype.connect = function connect(...args) {
  {
    let [options2, connectListener] = __intrinsic__isArray(args[0]) && args[0][normalizedArgsSymbol] ? args[0] : normalizeArgs(args), connection = this[ksocket], upgradeDuplex = !1, { port, host, path: path2, socket, rejectUnauthorized, checkServerIdentity, session, fd, pauseOnConnect } = options2;
    ...
    if (typeof bunTLS === "function") {
      if (tls = bunTLS.__intrinsic__call(this, port, host, !0), this._requestCert = !0, tls) {
  ...
  let [options, cb] = __intrinsic__isArray(args[0]) && args[0][normalizedArgsSymbol] ? args[0] : normalizeArgs(args);
...
Socket.prototype.setNoDelay = function setNoDelay(enable = !0) {
  if (enable = Boolean(enable === __intrinsic__undefined ? !0 : enable), !this._handle)
...
Socket.prototype.setTimeout = {
  setTimeout(msecs, callback) {
    ...
    } else if (this[kTimeout] = setTimeout(this._onTimeout.bind(this), msecs).unref(), callback !== __intrinsic__undefined)
...
Socket.prototype._write = function _write(chunk, encoding, callback) {
  ...
  let socket = this._handle;
  if (!socket)
    return callback(__intrinsic__makeErrorWithCode(220)), !1;
  this._unrefTimer();
  let success = socket.__intrinsic__write(chunk, encoding);
...
function lookupAndConnect(self, options) {
  ...
  lookup(host, dnsopts, function emitLookup(err, ip, addressType2) {
    ...
    if (err)
      ...
      err = __intrinsic__makeErrorWithCode(128, ip), process.nextTick(destroyNT, self, err);
    else if (addressType2 !== 4 && addressType2 !== 6)
      err = __intrinsic__makeErrorWithCode(117, addressType2, options.host, options.port), process.nextTick(destroyNT, self, err);
    else
...
function lookupAndConnectMultiple(self, lookup, host, options, dnsopts, port, localAddress, localPort, timeout) {
  lookup(host, dnsopts, function emitLookup(err, addresses) {
    if (!self.connecting)
      ...
      self.emit("lookup", err, __intrinsic__undefined, __intrinsic__undefined, host), process.nextTick(destroyNT, self, err);
      return;
    }
    ...
    for (let i = 0, l = addresses.length;i < l; i++) {
      ...
      if (isIP(ip) && (addressType === 4 || addressType === 6)) {
        ...
        if (!ArrayPrototypeIncludes.__intrinsic__call(validIps[destination], ip))
          ArrayPrototypePush.__intrinsic__call(validAddresses[destination], address2), ArrayPrototypePush.__intrinsic__call(validIps[destination], ip);
      }
    }
    if (!validAddresses[0].length && !validAddresses[1].length) {
      let { address: firstIp, family: firstAddressType } = addresses[0];
      if (!isIP(firstIp))
        err = __intrinsic__makeErrorWithCode(128, firstIp), process.nextTick(destroyNT, self, err);
      else if (firstAddressType !== 4 && firstAddressType !== 6)
        err = __intrinsic__makeErrorWithCode(117, firstAddressType, options.host, options.port), process.nextTick(destroyNT, self, err);
      return;
    }
    let toAttempt = [];
...
function internalConnect(self, options, address2, port, addressType, localAddress, localPort, _flags) {
  ...
  let tls = __intrinsic__undefined, bunTLS = self[bunTlsSymbol];
  if (typeof bunTLS === "function") {
    if (tls = bunTLS.__intrinsic__call(self, port, self._host, !0), self._requestCert = !0, tls) {
...
function internalConnectMultiple(context, canceled) {
  ...
  if (localPort) {
    ...
    if (err = checkBindError(err, localPort, self._handle), err) {
      ArrayPrototypePush.__intrinsic__call(context.errors, new ExceptionWithHostPort(err, "bind", localAddress, localPort)), internalConnectMultiple(context);
      return;
    }
  }
  if (self.blockList?.check(address2, `ipv${addressType}`)) {
    let ex = __intrinsic__makeErrorWithCode(142, address2);
    ArrayPrototypePush.__intrinsic__call(context.errors, ex), self.emit("connectionAttemptFailed", address2, port, addressType, ex), internalConnectMultiple(context);
    return;
  }
  let connection = self[ksocket];
  ...
  let req = {};
  if (req.oncomplete = afterConnectMultiple.bind(__intrinsic__undefined, context, current), req.address = address2, req.port = port, req.localAddress = localAddress, req.localPort = localPort, req.addressType = addressType, req.tls = tls, ArrayPrototypePush.__intrinsic__call(self.autoSelectFamilyAttemptedAddresses, `${address2}:${port}`), err = kConnectTcp(self, addressType, req, address2, port), err) {
    let ex = new ExceptionWithHostPort(err, "connect", address2, port);
    ArrayPrototypePush.__intrinsic__call(context.errors, ex), self.emit("connectionAttemptFailed", address2, port, addressType, ex), internalConnectMultiple(context);
    return;
  }
...
function internalConnectMultipleTimeout(context, req, handle) {
  if (context.socket.emit("connectionAttemptTimeout", req.address, req.port, req.addressType), req.oncomplete = __intrinsic__undefined, ArrayPrototypePush.__intrinsic__call(context.errors, createConnectionError(req, UV_ETIMEDOUT)), handle.close(), context.socket.connecting)
...
function afterConnectMultiple(context, current, status, handle, req, readable, writable) {
  ...
  if (status !== 0) {
    ...
    if (ArrayPrototypePush.__intrinsic__call(context.errors, ex), self.emit("connectionAttemptFailed", req.address, req.port, req.addressType, ex), context.socket.connecting)
...
function Server(options, connectionListener) {
  ...
  if (EventEmitter.__intrinsic__apply(this, []), typeof options === "function")
...
Server.prototype.listen = function listen(port, hostname, onListen) {
  ...
  if (typeof port === "string") {
    if (Number.isSafeInteger(hostname)) {
      ...
      onListen = hostname;
    path = port, hostname = __intrinsic__undefined, port = __intrinsic__undefined;
  } else {
    if (typeof hostname === "number")
      backlog = hostname, hostname = __intrinsic__undefined;
    else if (typeof hostname === "function")
      onListen = hostname, hostname = __intrinsic__undefined;
    else if (typeof hostname === "string" && typeof onListen === "number")
      backlog = onListen, onListen = argsLength > 3 ? arguments[3] : __intrinsic__undefined;
      ...
      if (!Number.isSafeInteger(port) || port < 0)
        if (path) {
          ...
          if (isLinux && isAbstractPath && (options.writableAll || options.readableAll)) {
            let message = `The argument 'options' can not set readableAll or writableAll to true when path is abstract unix socket. Received ${JSON.stringify(options)}`, error = __intrinsic__makeTypeError(message);
            throw error.code = "ERR_INVALID_ARG_VALUE", error;
          }
          hostname = path, port = __intrinsic__undefined;
        } else {
          let message = `The argument 'options' must have the property "port" or "path"`;
          try {
            message = `${message}. Received ${JSON.stringify(options)}`;
          } catch {}
          let error = __intrinsic__makeTypeError(message);
          throw error.code = "ERR_INVALID_ARG_VALUE", error;
        }
  ...
  try {
    var tls = __intrinsic__undefined, TLSSocketClass = __intrinsic__undefined;
    let bunTLS = this[bunTlsSymbol], options = this[bunSocketServerOptions], contexts = null;
    if (typeof bunTLS === "function") {
      if ([tls, TLSSocketClass] = bunTLS.__intrinsic__call(this, port, hostname, !1), options.servername = tls.serverName, options[kSocketClass] = TLSSocketClass, contexts = tls.contexts, !tls.requestCert)
...
Server.prototype[kRealListen] = function(path, port, hostname, exclusive, ipv6Only, allowHalfOpen, reusePort, tls, contexts, _onListen, fd) {
  ...
  if (addr && typeof addr === "object") {
    let familyLast = __intrinsic__String(addr.family).slice(-1);
...
function listenInCluster(server, address3, port, addressType, backlog, fd, exclusive, ipv6Only, allowHalfOpen, reusePort, flags, options, path, hostname, tls, contexts, onListen) {
  if (exclusive = !!exclusive, cluster === __intrinsic__undefined)
