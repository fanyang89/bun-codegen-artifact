// @bun
// build/release/tmp_modules/node/crypto.ts
var $, StringDecoder = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 143) || __intrinsic__createInternalModuleById(143)).StringDecoder, LazyTransform = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 49) || __intrinsic__createInternalModuleById(49), { defineCustomPromisifyArgs } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 31) || __intrinsic__createInternalModuleById(31), Writable = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 59) || __intrinsic__createInternalModuleById(59), { CryptoHasher } = Bun, {
  getCurves,
  certVerifySpkac,
  certExportPublicKey,
  certExportChallenge,
  ...
  scrypt,
  scryptSync
} = __intrinsic__lazy(46), normalizeEncoding = __intrinsic__lazy(37), { validateString } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), kHandle = Symbol("kHandle");
...
var Buffer = globalThis.Buffer, { isAnyArrayBuffer, isArrayBufferView } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 144) || __intrinsic__createInternalModuleById(144);
function getArrayBufferOrView(buffer, name, encoding) {
  if (buffer instanceof KeyObject) {
    if (buffer.type !== "secret") {
      let error2 = __intrinsic__makeTypeError(`ERR_CRYPTO_INVALID_KEY_OBJECT_TYPE: Invalid key object type ${key.type}, expected secret`);
...
function pbkdf2(password, salt, iterations, keylen, digest, callback) {
  if (typeof digest === "function")
    callback = digest, digest = __intrinsic__undefined;
...
function Sign(algorithm, options) {
  ...
  validateString(algorithm, "algorithm"), this[kHandle] = new _Sign, this[kHandle].init(algorithm), Writable.__intrinsic__apply(this, [options]);
}
...
function Verify(algorithm, options) {
  ...
  validateString(algorithm, "algorithm"), this[kHandle] = new _Verify, this[kHandle].init(algorithm), Writable.__intrinsic__apply(this, [options]);
}
...
function Hash(algorithm, options) {
  ...
  this[kHandle] = handle, LazyTransform.__intrinsic__apply(this, [options]);
}
...
};
{
  let getDecoder2 = function(decoder, encoding) {
    ...
    this[kHandle] = new Cipher(!1, cipher, key2, iv, options), LazyTransform.__intrinsic__apply(this, [options]), this._decoder = null;
  }, Decipheriv2 = function(cipher, key2, iv, options) {
    if (!new.target)
      return new Decipheriv2(cipher, key2, iv, options);
    this[kHandle] = new Cipher(!0, cipher, key2, iv, options), LazyTransform.__intrinsic__apply(this, [options]), this._decoder = null;
  };
  getDecoder = getDecoder2, Cipheriv = Cipheriv2, Decipheriv = Decipheriv2, __intrinsic__toClass(Cipheriv2, "Cipheriv", LazyTransform), Object.assign(Cipheriv2.prototype, {
  ...
  }), __intrinsic__toClass(Decipheriv2, "Decipheriv", LazyTransform), Object.assign(Decipheriv2.prototype, {
