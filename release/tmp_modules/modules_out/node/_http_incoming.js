// @bun
// build/release/tmp_modules/node/_http_incoming.ts
var $, Readable = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 55) || __intrinsic__createInternalModuleById(55), {
  ...
  kAbortController
} = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 25) || __intrinsic__createInternalModuleById(25), { FakeSocket } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 26) || __intrinsic__createInternalModuleById(26), defaultIncomingOpts = { type: "request" }, nop = () => {};
function assignHeadersSlow(object, req) {
  ...
  for (let key in headers) {
    ...
    if (key = key.toLowerCase(), key !== "set-cookie")
      value = __intrinsic__String(value), __intrinsic__putByValDirect(rawHeaders, i++, originalKey), __intrinsic__putByValDirect(rawHeaders, i++, value), outHeaders[key] = value;
    else if (__intrinsic__isJSArray(value)) {
      outHeaders[key] = value.slice();
      for (let entry of value)
        __intrinsic__putByValDirect(rawHeaders, i++, originalKey), __intrinsic__putByValDirect(rawHeaders, i++, entry);
    } else
      value = __intrinsic__String(value), outHeaders[key] = [value], __intrinsic__putByValDirect(rawHeaders, i++, originalKey), __intrinsic__putByValDirect(rawHeaders, i++, value);
  }
...
function assignHeaders(object, req) {
  if (assignHeadersFast(req, object, headersTuple)) {
    let headers = __intrinsic__getInternalField(headersTuple, 0), rawHeaders = __intrinsic__getInternalField(headersTuple, 1);
    return __intrinsic__putInternalField(headersTuple, 0, __intrinsic__undefined), __intrinsic__putInternalField(headersTuple, 1, __intrinsic__undefined), object.headers = headers, object.rawHeaders = rawHeaders, !0;
  } else
...
function IncomingMessage(req, options = defaultIncomingOpts) {
  if (this[abortedSymbol] = !1, this[eofInProgress] = !1, this._consuming = !1, this._dumped = !1, this.complete = !1, this._closed = !1, req === kHandle) {
    if (this[typeSymbol] = NodeHTTPIncomingRequestType.NodeHTTPResponse, this.url = arguments[1], this.method = arguments[2], this.headers = arguments[3], this.rawHeaders = arguments[4], this[kHandle] = arguments[5], this[noBodySymbol] = !arguments[6], this[fakeSocketSymbol] = arguments[7], Readable.__intrinsic__call(this), arguments[6])
      this.on("pause", onIncomingMessagePauseNodeHTTPResponse), this.on("resume", onIncomingMessageResumeNodeHTTPResponse);
  } else {
    this[noBodySymbol] = !1, Readable.__intrinsic__call(this);
    var { [typeSymbol]: type } = options || {};
    this[webRequestOrResponse] = req, this[typeSymbol] = type, this[bodyStreamSymbol] = __intrinsic__undefined;
...
var IncomingMessagePrototype = {
  ...
  _destroy: function IncomingMessage_destroy(err, cb) {
    ...
    if (nodeHTTPResponse) {
      if (this[kHandle] = __intrinsic__undefined, nodeHTTPResponse.onabort = nodeHTTPResponse.ondata = __intrinsic__undefined, !nodeHTTPResponse.finished && shouldEmitAborted)
      ...
      let streamState = stream?.__intrinsic__state;
