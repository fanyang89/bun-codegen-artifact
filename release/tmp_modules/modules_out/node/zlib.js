// @bun
// build/release/tmp_modules/node/zlib.ts
var $, BufferModule = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 141) || __intrinsic__createInternalModuleById(141), crc32 = __intrinsic__lazy(91), NativeZlib = __intrinsic__lazy(92), NativeBrotli = __intrinsic__lazy(93), NativeZstd = __intrinsic__lazy(94), ObjectKeys = Object.keys, ArrayPrototypePush = __intrinsic__Array.prototype.push, ObjectDefineProperty = Object.defineProperty, ObjectDefineProperties = Object.defineProperties, ObjectFreeze = Object.freeze, TypedArrayPrototypeFill = __intrinsic__Uint8Array.prototype.fill, ArrayPrototypeForEach = __intrinsic__Array.prototype.forEach, NumberIsNaN = Number.isNaN, MathMax = Math.max, ArrayBufferIsView = __intrinsic__ArrayBuffer.isView, isArrayBufferView = ArrayBufferIsView, isAnyArrayBuffer = (b) => b instanceof __intrinsic__ArrayBuffer || b instanceof SharedArrayBuffer, kMaxLength = __intrinsic__requireMap.__intrinsic__get("buffer")?.exports.kMaxLength ?? BufferModule.kMaxLength, { Transform, finished } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117) || __intrinsic__createInternalModuleById(117), owner_symbol = Symbol("owner_symbol"), { checkRangesOrGetDefault, validateFunction, validateFiniteNumber } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), kFlushFlag = Symbol("kFlushFlag"), kError = Symbol("kError"), { zlib: constants } = process.binding("constants"), {
  Z_NO_FLUSH,
  Z_BLOCK,
  Z_PARTIAL_FLUSH,
  Z_SYNC_FLUSH,
...
function zlibBufferOnData(chunk) {
  ...
  if (this.nread += chunk.length, this.nread > this._maxOutputLength)
    this.close(), this.removeAllListeners("end"), this.cb(__intrinsic__makeErrorWithCode(13, this._maxOutputLength));
}
...
function ZlibBase(opts, mode, handle, { flush, finishFlush, fullFlush }) {
  ...
  Transform.__intrinsic__apply(this, [{ autoDestroy: !0, ...opts }]), this[kError] = null, this.bytesWritten = 0, this._handle = handle, handle[owner_symbol] = this, handle.onerror = zlibOnError, this._outBuffer = __intrinsic__Buffer.allocUnsafe(chunkSize), this._outOffset = 0, this._chunkSize = chunkSize, this._defaultFlushFlag = flush, this._finishFlushFlag = finishFlush, this._defaultFullFlushFlag = fullFlush, this._info = opts && opts.info, this._maxOutputLength = maxOutputLength;
}
...
var kFlushBuffers = [];
{
  let dummyArrayBuffer = new __intrinsic__ArrayBuffer;
  for (let flushFlag of kFlushFlagList)
    kFlushBuffers[flushFlag] = __intrinsic__Buffer.from(dummyArrayBuffer), kFlushBuffers[flushFlag][kFlushFlag] = flushFlag;
}
  ...
  while (!0) {
    if (handle.writeSync(flushFlag, chunk, inOff, availInBefore, buffer, offset, availOutBefore), error) {
      if (typeof error === "string")
        ...
        error = Error(__intrinsic__String(error));
      throw error;
    } else if (self[kError])
      throw self[kError];
    ...
    if (have > 0) {
      ...
      if (offset += have, ArrayPrototypePush.__intrinsic__call(buffers, out), nread += out.byteLength, nread > self._maxOutputLength)
        throw _close(self), __intrinsic__makeErrorWithCode(13, self._maxOutputLength);
    }
    if (availOutAfter === 0 || offset >= chunkSize)
      availOutBefore = chunkSize, offset = 0, buffer = __intrinsic__Buffer.allocUnsafe(chunkSize);
    if (availOutAfter === 0)
...
function processCallback() {
  ...
  if (availOutAfter === 0 || self._outOffset >= self._chunkSize)
    handle.availOutBefore = self._chunkSize, self._outOffset = 0, self._outBuffer = __intrinsic__Buffer.allocUnsafe(self._chunkSize);
  if (availOutAfter === 0) {
...
function Zlib(opts, mode) {
  ...
  this._writeState = new Uint32Array(2), handle.init(windowBits, level, memLevel, strategy, this._writeState, processCallback, dictionary), ZlibBase.__intrinsic__apply(this, [opts, mode, handle, zlibDefaultOpts]), this._level = level, this._strategy = strategy;
}
...
function Brotli(opts, mode) {
  if (TypedArrayPrototypeFill.__intrinsic__call(brotliInitParamsArray, -1), opts?.params)
...
class Zstd extends ZlibBase {
  constructor(opts, mode, initParamsArray, maxParam) {
    ...
    let handle = new NativeZstd(mode), pledgedSrcSize = opts?.pledgedSrcSize ?? __intrinsic__undefined, writeState = new Uint32Array(2);
