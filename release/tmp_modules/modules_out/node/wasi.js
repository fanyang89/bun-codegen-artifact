// @bun
// build/release/tmp_modules/node/wasi.ts
var $, nodeFsConstants = __intrinsic__processBindingConstants.fs, __getOwnPropNames = Object.getOwnPropertyNames, __commonJS = (cb, mod = __intrinsic__undefined) => function __require2() {
  ...
  "node_modules/wasi-js/dist/wasi.js"(exports) {
    ...
    function getDefaults() {
      ...
      let defaultBindings = {
        ...
        isTTY: (fd) => (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 124) || __intrinsic__createInternalModuleById(124)).isatty(fd),
        fs: __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 98) || __intrinsic__createInternalModuleById(98),
        path: __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 107) || __intrinsic__createInternalModuleById(107)
      };
      return defaultConfig = {
        args: [],
        env: {},
      ...
      constructor(wasiConfig = {}) {
        ...
        let getiovs = (iovs, iovsLen) => {
          ...
          if (iovsLen === 1) {
            ...
            try {
              return [new __intrinsic__Uint8Array(buffer, buf, bufLen)];
            } catch (err) {
          ...
          for (let i = 0, ptr = iovs;i < iovsLen; i++, ptr += 8) {
            ...
            try {
              buffers[i] = new __intrinsic__Uint8Array(buffer, buf, bufLen);
            } catch (err) {
              throw console.warn("WASI.getiovs -- invalid buffer", err), new types_1.WASIError(constants_1.WASI_EINVAL);
            }
          }
        ...
        if (this.wasiImport = {
          args_get: (argv, argvBuf) => {
            ...
            return args.forEach((a) => {
              this.view.setUint32(coffset, offset, !0), coffset += 4, offset += __intrinsic__Buffer.from(this.memory.buffer).write(`${a}\x00`, offset);
            }), constants_1.WASI_ESUCCESS;
          },
          args_sizes_get: (argc, argvBufSize) => {
            this.refreshMemory(), this.view.setUint32(argc, args.length, !0);
            let size = args.reduce((acc, a) => acc + __intrinsic__Buffer.byteLength(a) + 1, 0);
          ...
          environ_get: (environ, environBuf) => {
            ...
            return Object.entries(this.env).forEach(([key, value]) => {
              this.view.setUint32(coffset, offset, !0), coffset += 4, offset += __intrinsic__Buffer.from(this.memory.buffer).write(`${key}=${value}\x00`, offset);
            }), constants_1.WASI_ESUCCESS;
          },
          environ_sizes_get: (environCount, environBufSize) => {
            this.refreshMemory();
            let envProcessed = Object.entries(this.env).map(([key, value]) => `${key}=${value}\x00`), size = envProcessed.reduce((acc, e) => acc + __intrinsic__Buffer.byteLength(e), 0);
          ...
          fd_prestat_get: wrap((fd, bufPtr) => {
            let stats = CHECK_FD(fd, BigInt(0));
            return this.refreshMemory(), this.view.setUint8(bufPtr, constants_1.WASI_PREOPENTYPE_DIR), this.view.setUint32(bufPtr + 4, __intrinsic__Buffer.byteLength(stats.fakePath ?? stats.path ?? ""), !0), constants_1.WASI_ESUCCESS;
          }),
          fd_prestat_dir_name: wrap((fd, pathPtr, pathLen) => {
            let stats = CHECK_FD(fd, BigInt(0));
            return this.refreshMemory(), __intrinsic__Buffer.from(this.memory.buffer).write(stats.fakePath ?? stats.path ?? "", pathPtr, pathLen, "utf8"), constants_1.WASI_ESUCCESS;
          }),
          ...
          fd_readdir: wrap((fd, bufPtr, bufLen, cookie, bufusedPtr) => {
            ...
            for (let i = Number(cookie);i < entries.length; i += 1) {
              let entry = entries[i], nameLength = __intrinsic__Buffer.byteLength(entry.name);
              ...
              if (this.view.setUint8(bufPtr, filetype), bufPtr += 1, bufPtr += 3, bufPtr + nameLength >= startPtr + bufLen)
                break;
              __intrinsic__Buffer.from(this.memory.buffer).write(entry.name, bufPtr), bufPtr += nameLength;
            }
          ...
          path_create_directory: wrap((fd, pathPtr, pathLen) => {
            ...
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let p = __intrinsic__Buffer.from(this.memory.buffer, pathPtr, pathLen).toString();
          ...
          path_filestat_get: wrap((fd, flags, pathPtr, pathLen, bufPtr) => {
            ...
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let p = __intrinsic__Buffer.from(this.memory.buffer, pathPtr, pathLen).toString(), rstats;
            if (flags)
          ...
          path_link: wrap((oldFd, _oldFlags, oldPath, oldPathLen, newFd, newPath, newPathLen) => {
            ...
            if (!ostats.path || !nstats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let op = __intrinsic__Buffer.from(this.memory.buffer, oldPath, oldPathLen).toString(), np = __intrinsic__Buffer.from(this.memory.buffer, newPath, newPathLen).toString();
          ...
          path_readlink: wrap((fd, pathPtr, pathLen, buf, bufLen, bufused) => {
            ...
            if (!stats.path)
              return constants_1.WASI_EINVAL;
            this.refreshMemory();
            let p = __intrinsic__Buffer.from(this.memory.buffer, pathPtr, pathLen).toString(), full = path.resolve(stats.path, p), r = fs.readlinkSync(full), used = __intrinsic__Buffer.from(this.memory.buffer).write(r, buf, bufLen);
