// @bun
// build/release/tmp_modules/node/fs.ts
var $, EventEmitter = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 96) || __intrinsic__createInternalModuleById(96), promises = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 97) || __intrinsic__createInternalModuleById(97), types = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 144) || __intrinsic__createInternalModuleById(144), { validateFunction, validateInteger } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), kEmptyObject = Object.freeze(Object.create(null)), isDate = types.isDate, { fs } = promises.__intrinsic__data, constants = __intrinsic__processBindingConstants.fs, _lazyGlob;
function lazyGlob() {
...
var access = function access2(path, mode, callback) {
  if (__intrinsic__isCallable(mode))
    callback = mode, mode = __intrinsic__undefined;
  ...
  try {
    fs.exists.__intrinsic__apply(fs, [path]).then((existed) => callback(existed), (_) => callback(!1));
  } catch {
    callback(!1);
  }
...
}, lchmod = constants.O_SYMLINK !== __intrinsic__undefined ? function lchmod2(path, mode, callback) {
  ensureCallback(callback), fs.lchmod(path, mode).then(nullcallback(callback), callback);
} : __intrinsic__undefined, lchown = function lchown2(path, uid, gid, callback) {
  ...
  if (arguments.length <= 4) {
    if (arguments.length === 4) {
      ...
      if (!types.isArrayBufferView(buffer))
        params = buffer, { buffer = __intrinsic__Buffer.alloc(16384) } = params ?? {};
      callback = offsetOrOptions;
    } else
      callback = buffer, buffer = __intrinsic__Buffer.alloc(16384);
  ...
  if (!__intrinsic__isCallable(position)) {
    if (__intrinsic__isCallable(offsetOrOptions))
      position = offsetOrOptions, offsetOrOptions = __intrinsic__undefined;
    else
      position = length;
    length = "utf8";
  }
  ...
  if (callback === __intrinsic__undefined)
    callback = type, ensureCallback(callback), type = __intrinsic__undefined;
  ...
  callback = ensureCallback(callback), fs.writev(fd, buffers, position).__intrinsic__then((bytesWritten) => callback(null, bytesWritten, buffers), callback);
}, writevSync = fs.writevSync.bind(fs), readv = function readv2(fd, buffers, position, callback) {
  if (typeof position === "function")
    callback = position, position = null;
  callback = ensureCallback(callback), fs.readv(fd, buffers, position).__intrinsic__then((bytesRead) => callback(null, bytesRead, buffers), callback);
...
}, { defineCustomPromisifyArgs } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 31) || __intrinsic__createInternalModuleById(31), kCustomPromisifiedSymbol = Symbol.for("nodejs.util.promisify.custom"), existsCb = exists;
...
class Dir {
  ...
  constructor(handle, path, options) {
    ...
    validateInteger(handle, "handle", 0), this.#handle = __intrinsic__toLength(handle), this.#path = path, this.#options = options;
  }
  readSync() {
    if (this.#handle < 0)
...
function glob(pattern, options, callback) {
  if (typeof options === "function")
    callback = options, options = __intrinsic__undefined;
  validateFunction(callback, "callback"), __intrinsic__Array.fromAsync(lazyGlob().glob(pattern, options ?? kEmptyObject)).then((result) => callback(null, result)).catch(callback);
}
...
function setName(fn, value) {
  Object.__intrinsic__defineProperty(fn, "name", { value, enumerable: !1, configurable: !0 });
}
