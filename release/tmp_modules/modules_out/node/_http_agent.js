// @bun
// build/release/tmp_modules/node/_http_agent.ts
var $, EventEmitter = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 96) || __intrinsic__createInternalModuleById(96), { parseProxyConfigFromEnv, kProxyConfig, checkShouldUseProxy, kWaitForProxyTunnel } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 25) || __intrinsic__createInternalModuleById(25), { getLazy, kEmptyObject, once } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 32) || __intrinsic__createInternalModuleById(32), { validateNumber, validateOneOf, validateString } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), { isIP } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 28) || __intrinsic__createInternalModuleById(28), kOnKeylog = Symbol("onkeylog"), kRequestOptions = Symbol("requestOptions");
...
function Agent(options) {
  ...
  if (EventEmitter.__intrinsic__call(this), this.options = { __proto__: null, ...options }, this.defaultPort = this.options.defaultPort || 80, this.protocol = this.options.protocol || "http:", this.options.noDelay === __intrinsic__undefined)
  ...
  if (validateOneOf(this.scheduling, "scheduling", ["fifo", "lifo"]), this.maxTotalSockets !== __intrinsic__undefined)
    validateNumber(this.maxTotalSockets, "maxTotalSockets", 1);
  else
    this.maxTotalSockets = __intrinsic__Infinity;
...
var tls = getLazy(() => __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 122) || __intrinsic__createInternalModuleById(122)), net = getLazy(() => __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 104) || __intrinsic__createInternalModuleById(104));
Agent.defaultMaxSockets = __intrinsic__Infinity;
...
Agent.prototype.createSocket = function createSocket(req, options, cb) {
  ...
  let timeout = req.timeout || this.options.timeout || __intrinsic__undefined;
  if (timeout)
...
Agent.prototype.removeSocket = function removeSocket(s, options) {
  ...
  if (req && options)
    req[kRequestOptions] = __intrinsic__undefined, this.createSocket(req, options, (err, socket) => {
      if (err) {
...
function setRequestSocket(agent, req, socket) {
  ...
  if (req.timeout === __intrinsic__undefined || req.timeout === agentTimeout)
    return;
...
$ = {
  ...
  globalAgent: new Agent({
    ...
    scheduling: "lifo",
    timeout: 5000,
    proxyEnv: __intrinsic__undefined
  })
};
$$EXPORT$$($).$$EXPORT_END$$;
