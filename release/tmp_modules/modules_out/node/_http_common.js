// @bun
// build/release/tmp_modules/node/_http_common.ts
var $, { checkIsHttpToken } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), FreeList = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 19) || __intrinsic__createInternalModuleById(19), { methods, allMethods, HTTPParser } = process.binding("http_parser"), incoming = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 74) || __intrinsic__createInternalModuleById(74), { IncomingMessage, readStart, readStop } = incoming, RegExpPrototypeExec = __intrinsic__RegExp.prototype.exec, headerCharRegex;
...
var validateHeaderName = (name, label) => {
  if (typeof name !== "string" || !name || !checkIsHttpToken(name))
    throw __intrinsic__makeErrorWithCode(127, label || "Header name", name);
}, validateHeaderValue = (name, value) => {
  if (value === __intrinsic__undefined)
    throw __intrinsic__makeErrorWithCode(71, value, name);
  if (checkInvalidHeaderChar(value))
    throw __intrinsic__makeErrorWithCode(121, "header content", name);
