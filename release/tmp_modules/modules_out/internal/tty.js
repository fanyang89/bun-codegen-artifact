// @bun
// build/release/tmp_modules/internal/tty.ts
var $;
var TERM_ENVS = {
  eterm: 4,
  cons25: 4,
...
function warnOnDeactivatedColors(env) {
  if (warned)
    return;
  let name = "";
  if (env.NODE_DISABLE_COLORS !== __intrinsic__undefined)
    name = "NODE_DISABLE_COLORS";
  if (env.NO_COLOR !== __intrinsic__undefined) {
    if (name !== "")
      name += "' and '";
    name += "NO_COLOR";
  }
...
function getColorDepth(env) {
  ...
  if (FORCE_COLOR !== __intrinsic__undefined)
    switch (FORCE_COLOR) {
      case "":
      case "1":
      case "true":
  ...
  if (env.NODE_DISABLE_COLORS !== __intrinsic__undefined || env.NO_COLOR !== __intrinsic__undefined || env.TERM === "dumb")
    return 1;
  if (env.TMUX)
    return 8;
  if (env.CI) {
