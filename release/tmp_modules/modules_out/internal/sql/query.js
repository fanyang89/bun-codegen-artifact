// @bun
// build/release/tmp_modules/internal/sql/query.ts
var $, _resolve = Symbol("resolve"), _reject = Symbol("reject"), _handle = Symbol("handle"), _queryStatus = Symbol("status"), _handler = Symbol("handler"), _strings = Symbol("strings"), _values = Symbol("values"), _flags = Symbol("flags"), _results = Symbol("results"), _adapter = Symbol("adapter"), PublicPromise = __intrinsic__Promise;
...
class Query extends PublicPromise {
  ...
  async#runAsync() {
    ...
    if (!handle)
      return this;
    await __intrinsic__Promise.__intrinsic__resolve();
    try {
      return handler(this, handle);
    } catch (err) {
      this[_queryStatus] |= 8 /* error */, this.reject(err);
  ...
  #runAsyncAndCatch() {
    ...
    if (__intrinsic__isPromise(runPromise) && runPromise !== this)
      runPromise.catch(() => {});
  }
  then() {
    this.#runAsyncAndCatch();
    let result = super.__intrinsic__then.__intrinsic__apply(this, arguments);
    if (arguments.length >= 2 && arguments[1] != null)
      __intrinsic__markPromiseAsHandled(result);
    return result;
  }
  catch() {
    if (this[_flags] & 16 /* notTagged */)
      throw this[_adapter].notTaggedCallError();
    this.#runAsyncAndCatch();
    let result = super.catch.__intrinsic__apply(this, arguments);
    return __intrinsic__markPromiseAsHandled(result), result;
  }
  finally(_onfinally) {
    if (this[_flags] & 16 /* notTagged */)
      throw this[_adapter].notTaggedCallError();
    return this.#runAsyncAndCatch(), super.finally.__intrinsic__apply(this, arguments);
  }
}
