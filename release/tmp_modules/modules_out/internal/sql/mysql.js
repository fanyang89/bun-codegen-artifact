// @bun
// build/release/tmp_modules/internal/sql/mysql.ts
...
} = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 36) || __intrinsic__createInternalModuleById(36), { MySQLError } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 33) || __intrinsic__createInternalModuleById(33), {
...
class MySQLAdapter {
  ...
  constructor(connectionInfo) {
    this.connectionInfo = connectionInfo, this.connections = new __intrinsic__Array(connectionInfo.max);
  }
  ...
  flushConcurrentQueries() {
    ...
    if (maxDistribution === 0)
      return;
    while (!0) {
      let nonReservedConnections = __intrinsic__Array.from(this.readyConnections).filter((c) => !(c.flags & 4 /* preReserved */) && c.queryCount < maxDistribution);
  ...
  async#close() {
    ...
    return this.readyConnections.clear(), this.waitingQueue.length = 0, __intrinsic__Promise.all(promises);
  }
  async close(options) {
    if (this.closed)
      return;
    ...
    if (timeout) {
      ...
      let { promise, resolve } = __intrinsic__Promise.withResolvers(), timer = setTimeout(() => {
  ...
  connect(onConnected, reserved = !1) {
    ...
    if (reserved) {
      let connectionWithLeastQueries = null, leastQueries = __intrinsic__Infinity;
