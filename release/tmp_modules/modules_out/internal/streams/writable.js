// @bun
// build/release/tmp_modules/internal/streams/writable.ts
var $, EE = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 96) || __intrinsic__createInternalModuleById(96), { Stream } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 50) || __intrinsic__createInternalModuleById(50), destroyImpl = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 43) || __intrinsic__createInternalModuleById(43), eos = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 47) || __intrinsic__createInternalModuleById(47), { addAbortSignal } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 41) || __intrinsic__createInternalModuleById(41), { getHighWaterMark, getDefaultHighWaterMark } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 56) || __intrinsic__createInternalModuleById(56), {
  kOnConstructed,
  kState,
  kObjectMode,
  kErrorEmitted,
  ...
  kErrored,
  kConstructed
} = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 58) || __intrinsic__createInternalModuleById(58), ObjectDefineProperties = Object.defineProperties, ArrayPrototypeSlice = __intrinsic__Array.prototype.slice, ObjectDefineProperty = Object.defineProperty, SymbolHasInstance = Symbol.hasInstance, FunctionPrototypeSymbolHasInstance = Function.prototype[Symbol.hasInstance], StringPrototypeToLowerCase = __intrinsic__String.prototype.toLowerCase, SymbolAsyncDispose = Symbol.asyncDispose, { errorOrDestroy } = destroyImpl;
function nop() {}
...
function WritableState(options, stream, isDuplex) {
  ...
  this.length = 0, this.corked = 0, this.onwrite = onwrite.bind(__intrinsic__undefined, stream), this.writelen = 0, resetBuffer(this), this.pendingcb = 0;
}
  ...
  if ((state[kState] & kObjectMode) === 0) {
    ...
    if (typeof chunk === "string") {
      if ((state[kState] & kDecodeStrings) !== 0)
        chunk = __intrinsic__Buffer.from(chunk, encoding), encoding = "buffer";
...
function errorBuffer(state) {
  ...
  if ((state[kState] & kBuffered) !== 0)
    for (let n = state.bufferedIndex;n < state.buffered.length; ++n) {
      ...
      state.length -= len, callback(state.errored ?? __intrinsic__makeErrorWithCode(228, "write"));
    }
  callFinishedCallbacks(state, state.errored ?? __intrinsic__makeErrorWithCode(228, "end")), resetBuffer(state);
}
function clearBuffer(stream, state) {
  ...
  if (state[kState] |= kBufferProcessing, bufferedLength > 1 && stream._writev) {
    ...
    }, chunks = (state[kState] & kAllNoop) !== 0 && i === 0 ? buffered : ArrayPrototypeSlice.__intrinsic__call(buffered, i);
...
Writable.prototype.destroy = function(err, cb) {
  ...
  return destroy.__intrinsic__call(this, err, cb), this;
};
...
Writable.prototype[SymbolAsyncDispose] = function() {
  let error;
  if (!this.destroyed)
    error = this.writableFinished ? null : __intrinsic__makeAbortError(), this.destroy(error);
