// @bun
// build/release/tmp_modules/internal/streams/readable.ts
var $, EE = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 96) || __intrinsic__createInternalModuleById(96), { Stream, prependListener } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 50) || __intrinsic__createInternalModuleById(50), { addAbortSignal } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 41) || __intrinsic__createInternalModuleById(41), eos = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 47) || __intrinsic__createInternalModuleById(47), destroyImpl = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 43) || __intrinsic__createInternalModuleById(43), { getHighWaterMark, getDefaultHighWaterMark } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 56) || __intrinsic__createInternalModuleById(56), {
  kOnConstructed,
  kState,
  kObjectMode,
  kErrorEmitted,
  ...
  kErrored,
  kConstructed
} = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 58) || __intrinsic__createInternalModuleById(58), { aggregateTwoErrors } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 16) || __intrinsic__createInternalModuleById(16), { validateObject } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), { StringDecoder } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 143) || __intrinsic__createInternalModuleById(143), from = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 48) || __intrinsic__createInternalModuleById(48), { SafeSet } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 30) || __intrinsic__createInternalModuleById(30), { kAutoDestroyed } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 32) || __intrinsic__createInternalModuleById(32), ObjectDefineProperties = Object.defineProperties, SymbolAsyncDispose = Symbol.asyncDispose, NumberIsNaN = Number.isNaN, NumberIsInteger = Number.isInteger, NumberParseInt = Number.parseInt, ArrayPrototypeIndexOf = __intrinsic__Array.prototype.indexOf, ObjectKeys = Object.keys, SymbolAsyncIterator = Symbol.asyncIterator, TypedArrayPrototypeSet = __intrinsic__Uint8Array.prototype.set, { errorOrDestroy } = destroyImpl, nop = () => {}, kErroredValue = Symbol("kErroredValue"), kDefaultEncodingValue = Symbol("kDefaultEncodingValue"), kDecoderValue = Symbol("kDecoderValue"), kEncodingValue = Symbol("kEncodingValue"), kEnded = 512, kEndEmitted = 1024, kReading = 2048, kSync = 4096, kNeedReadable = 8192, kEmittedReadable = 16384, kReadableListening = 32768, kResumeScheduled = 65536, kMultiAwaitDrain = 131072, kReadingMore = 262144, kDataEmitted = 524288, kDefaultUTF8Encoding = 1048576, kDecoder = 2097152, kEncoding = 4194304, kHasFlowing = 8388608, kFlowing = 16777216, kHasPaused = 33554432, kPaused = 67108864, kDataListening = 134217728;
...
Readable.prototype[SymbolAsyncDispose] = function() {
  let error;
  if (!this.destroyed)
    error = this.readableEnded ? null : __intrinsic__makeAbortError(), this.destroy(error);
  return new __intrinsic__Promise((resolve, reject) => eos(this, (err) => err && err !== error ? reject(err) : resolve(null)));
};
...
function readableAddChunkUnshiftByteMode(stream, state, chunk, encoding) {
  ...
  if (typeof chunk === "string") {
    if (encoding ||= state.defaultEncoding, state.encoding !== encoding)
      if (state.encoding)
        chunk = __intrinsic__Buffer.from(chunk, encoding).toString(state.encoding);
      else
        chunk = __intrinsic__Buffer.from(chunk, encoding);
  } else if (Stream._isArrayBufferView(chunk))
    chunk = Stream._uint8ArrayToBuffer(chunk);
  else if (chunk !== __intrinsic__undefined && !(chunk instanceof __intrinsic__Buffer))
    return errorOrDestroy(stream, __intrinsic__makeErrorWithCode(118, "chunk", ["string", "Buffer", "TypedArray", "DataView"], chunk)), !1;
...
function readableAddChunkUnshiftValue(stream, state, chunk) {
  if ((state[kState] & kEndEmitted) !== 0)
    errorOrDestroy(stream, __intrinsic__makeErrorWithCode(234));
...
Readable.prototype.unpipe = function(dest) {
  ...
  let index = ArrayPrototypeIndexOf.__intrinsic__call(state.pipes, dest);
  if (index === -1)
    return this;
...
Readable.prototype.on = function(ev, fn) {
  let res = Stream.prototype.on.__intrinsic__call(this, ev, fn), state = this._readableState;
  if (ev === "data") {
...
Readable.prototype.removeListener = function(ev, fn) {
  let state = this._readableState, res = Stream.prototype.removeListener.__intrinsic__call(this, ev, fn);
  if (ev === "readable")
...
Readable.prototype.removeAllListeners = function(ev) {
  let res = Stream.prototype.removeAllListeners.__intrinsic__apply(this, arguments);
  if (ev === "readable" || ev === __intrinsic__undefined)
    process.nextTick(updateReadableListening, this);
  return res;
};
...
Readable.prototype.wrap = function(stream) {
  ...
  for (let j = 1;j < streamKeys.length; j++) {
    let i = streamKeys[j];
    if (this[i] === __intrinsic__undefined && typeof stream[i] === "function")
  ...
  try {
    while (!0) {
      ...
      if (chunk !== null)
        ...
        await new __intrinsic__Promise(next);
    }
  } catch (err) {
    throw error = aggregateTwoErrors(error, err), error;
  } finally {
    if ((error || options?.destroyOnReturn !== !1) && (error === __intrinsic__undefined || stream._readableState.autoDestroy))
...
function fromList(n, state) {
  ...
  if ((state[kState] & kObjectMode) !== 0)
    ...
    if ((state[kState] & kDecoder) !== 0) {
      ...
      ret = __intrinsic__Buffer.alloc(0);
    else if (len - idx === 1)
      ret = buf[idx], buf[idx++] = null;
    else {
      ret = __intrinsic__Buffer.allocUnsafe(state.length);
      let i = 0;
      while (idx < len)
        TypedArrayPrototypeSet.__intrinsic__call(ret, buf[idx], i), i += buf[idx].length, buf[idx++] = null;
    }
    ...
    ret = __intrinsic__Buffer.allocUnsafe(n);
    let retLen = n;
    while (idx < len) {
      let data = buf[idx];
      if (n > data.length)
        TypedArrayPrototypeSet.__intrinsic__call(ret, data, retLen - n), n -= data.length, buf[idx++] = null;
      else {
        if (n === data.length)
          TypedArrayPrototypeSet.__intrinsic__call(ret, data, retLen - n), buf[idx++] = null;
        else
          TypedArrayPrototypeSet.__intrinsic__call(ret, new __intrinsic__Buffer(data.buffer, data.byteOffset, n), retLen - n), buf[idx] = new __intrinsic__Buffer(data.buffer, data.byteOffset + n, data.length - n);
        break;
      }
    }
  }
