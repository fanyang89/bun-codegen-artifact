// @bun
// build/release/tmp_modules/internal/streams/pipeline.ts
var $, eos = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 47) || __intrinsic__createInternalModuleById(47), { once } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 32) || __intrinsic__createInternalModuleById(32), destroyImpl = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 43) || __intrinsic__createInternalModuleById(43), Duplex = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 44) || __intrinsic__createInternalModuleById(44), { aggregateTwoErrors } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 16) || __intrinsic__createInternalModuleById(16), { validateFunction, validateAbortSignal } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68) || __intrinsic__createInternalModuleById(68), {
  isIterable,
  isReadable,
  ...
  isReadableStream,
  isReadableFinished
} = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 58) || __intrinsic__createInternalModuleById(58), SymbolAsyncIterator = Symbol.asyncIterator, ArrayIsArray = __intrinsic__Array.isArray, SymbolDispose = Symbol.dispose, PassThrough, Readable, addAbortListener;
function destroyer(stream, reading, writing) {
  ...
  return {
    destroy: (err) => {
      if (finished)
        return;
      finished = !0, destroyImpl.destroyer(stream, err || __intrinsic__makeErrorWithCode(228, "pipe"));
    },
    cleanup
  };
}
...
function makeAsyncIterable(val) {
  ...
  throw __intrinsic__makeErrorWithCode(118, "val", ["Readable", "Iterable", "AsyncIterable"], val);
}
async function* fromReadable(val) {
  Readable ??= __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 55) || __intrinsic__createInternalModuleById(55), yield* Readable.prototype[SymbolAsyncIterator].__intrinsic__call(val);
}
async function pumpToNode(iterable, writable, finish, { end }) {
  ...
  }, wait = () => new __intrinsic__Promise((resolve, reject) => {
    if (error)
      reject(error);
    else
      onresolve = () => {
...
function pipelineImpl(streams, callback, opts) {
  ...
  function abort() {
    finishImpl(__intrinsic__makeAbortError(__intrinsic__undefined, { cause: outerSignal?.reason }));
  }
  ...
  for (let i = 0;i < streams.length; i++) {
    ...
    if (isNodeStream(stream)) {
      ...
      if (next !== null && (next?.closed || next?.destroyed))
        throw __intrinsic__makeErrorWithCode(233);
      if (end) {
    ...
    if (i === 0)
      if (typeof stream === "function") {
        if (ret = stream({ signal }), !isIterable(ret))
          throw __intrinsic__makeErrorWithCode(134, "Iterable, AsyncIterable or Stream", "source", ret);
      ...
      if (ret = stream(ret, { signal }), reading) {
        if (!isIterable(ret, !0))
          throw __intrinsic__makeErrorWithCode(134, "AsyncIterable", `transform[${i - 1}]`, ret);
      } else {
        PassThrough ??= __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 53) || __intrinsic__createInternalModuleById(53);
        let pt = new PassThrough({
          objectMode: !0
        }), then = ret?.then;
        if (typeof then === "function")
          finishCount++, then.__intrinsic__call(ret, (val) => {
          ...
          throw __intrinsic__makeErrorWithCode(134, "AsyncIterable or Promise", "destination", ret);
        ret = pt;
      ...
      if (isReadableNodeStream(ret)) {
        ...
        throw __intrinsic__makeErrorWithCode(118, "val", ["Readable", "Iterable", "AsyncIterable", "ReadableStream", "TransformStream"], ret);
      ret = stream;
...
function pipe(src, dst, finish, finishOnlyHandleError, { end }) {
  ...
  if (dst.on("close", () => {
    if (!ended)
      finishOnlyHandleError(__intrinsic__makeErrorWithCode(230));
