// @bun
// build/release/tmp_modules/thirdparty/node-fetch.ts
var $, bindings = __intrinsic__lazy(95), WebResponse = bindings[0], WebRequest = bindings[1], Blob = bindings[2], WebHeaders = bindings[3], FormData = bindings[4], File = bindings[5], nativeFetch = Bun.fetch;
...
class Headers extends WebHeaders {
  raw() {
    ...
    for (let key in obj) {
      let val = obj[key];
      if (!__intrinsic__isJSArray(val))
        obj[key] = [val];
    }
    return obj;
  }
  sort() {
    __intrinsic__throwTypeError("Expected this to be instanceof URLSearchParams");
  }
}
...
class Response extends WebResponse {
  ...
  constructor(body, init) {
    let { Readable, Stream } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117) || __intrinsic__createInternalModuleById(117);
  ...
  get body() {
    ...
    if (!body) {
      ...
      if (!web)
        return null;
      body = this[kBody] = new (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 69) || __intrinsic__createInternalModuleById(69))._ReadableFromWeb({}, web);
    }
    return body;
  }
  get headers() {
  ...
  async buffer() {
    return this.body, new __intrinsic__Buffer(await super.arrayBuffer());
  }
  async text() {
...
async function fetch(url, init) {
  if (init?.body && typeof init.body === "object" && !init.body[Symbol.asyncIterator]) {
    let { Readable, Stream, PassThrough } = __intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117) || __intrinsic__createInternalModuleById(117);
  ...
  let response = await nativeFetch.__intrinsic__call(__intrinsic__undefined, url, init);
