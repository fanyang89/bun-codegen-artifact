...
// Sourced from src/js/node/_http_server.ts


// Hardcoded module "node:_http_server"
const EventEmitter: typeof import("node:events").EventEmitter = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 96/*node:events*/) || __intrinsic__createInternalModuleById(96/*node:events*/));
const { Duplex, Stream } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 117/*node:stream*/) || __intrinsic__createInternalModuleById(117/*node:stream*/));
const {
  _checkInvalidHeaderChar: checkInvalidHeaderChar,
  validateHeaderName,
  validateHeaderValue,
} = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 73/*node:_http_common*/) || __intrinsic__createInternalModuleById(73/*node:_http_common*/));
const { validateObject, validateLinkHeaderValue, validateBoolean, validateInteger } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 68/*internal/validators*/) || __intrinsic__createInternalModuleById(68/*internal/validators*/));
const { ConnResetException } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 32/*internal/shared*/) || __intrinsic__createInternalModuleById(32/*internal/shared*/));
...
} = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 25/*internal/http*/) || __intrinsic__createInternalModuleById(25/*internal/http*/));
const NumberIsNaN = Number.isNaN;

const { format } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 66/*internal/util/inspect*/) || __intrinsic__createInternalModuleById(66/*internal/util/inspect*/));
...
const { kIncomingMessage } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 73/*node:_http_common*/) || __intrinsic__createInternalModuleById(73/*node:_http_common*/));
const kConnectionsCheckingInterval = Symbol("http.server.connectionsCheckingInterval");

const getBunServerAllClosedPromise = __intrinsic__lazy(40);
const sendHelper = __intrinsic__lazy(4);
...
function Server(options, callback): void {
  ...
  storeHTTPOptions.__intrinsic__call(this, options);
...
Server.prototype.close = function (optionalCallback?) {
  const server = this[serverSymbol];
  if (!server) {
    if (typeof optionalCallback === "function") process.nextTick(optionalCallback, __intrinsic__makeErrorWithCode(214, ));
    return;
  }
  this[serverSymbol] = undefined;
  const connectionsCheckingInterval = this[kConnectionsCheckingInterval];
...
Server.prototype[kRealListen] = function (tls, port, host, socketPath, reusePort, onListen) {
  {
    ...
    this[serverSymbol] = Bun.serve<any>({
      ...
      onNodeHTTPRequest(
        ...
        const { resolve, promise } = __intrinsic__newPromiseCapability(Promise);
    ...
    getBunServerAllClosedPromise(this[serverSymbol]).__intrinsic__then(emitCloseNTServer.bind(this));
...
function onServerClientError(ssl: boolean, socket: unknown, errorCode: number, rawPacket: ArrayBuffer) {
  ...
  switch (errorCode) {
    ...
    case HttpParserError.HTTP_PARSER_ERROR_INVALID_METHOD:
      err = __intrinsic__makeErrorWithCode(290, "Parse Error: Invalid method encountered");
    ...
    case HttpParserError.HTTP_PARSER_ERROR_REQUEST_HEADER_FIELDS_TOO_LARGE:
      err = __intrinsic__makeErrorWithCode(301, "Parse Error: Header overflow");
...
const NodeHTTPServerSocket = class Socket extends Duplex {
  ...
  #onCloseForDestroy(closeCallback) {
    this.#onClose();
    if (__intrinsic__isCallable(closeCallback)) closeCallback();
  }
  ...
  _destroy(err, callback) {
    const handle = this[kHandle];
    if (!handle) {
      if (__intrinsic__isCallable(callback)) callback(err);
      return;
    }
    handle.ondata = undefined;
    if (handle.closed) {
...
function _writeHead(statusCode, reason, obj, response) {
  ...
  if (checkInvalidHeaderChar(response.statusMessage)) throw __intrinsic__makeErrorWithCode(121, "statusMessage");
...
function ServerResponse(req, options): void {
  ...
  OutgoingMessage.__intrinsic__call(this, options);
...
function ServerResponse_finalDeprecated(chunk, encoding, callback) {
  ...
  if (!this.headersSent) {
    ...
    drainHeadersIfObservable.__intrinsic__call(this);
  ...
  ensureReadableStreamController.__intrinsic__call(this, controller => {
    ...
    if (__intrinsic__isPromise(prom)) prom.then(handler, handler);
    else handler();
  });
}
    ...
    new Response(
      new ReadableStream({
        ...
        pull: controller => {
          ...
          if (!this.finished) {
            const { promise, resolve } = __intrinsic__newPromiseCapability(GlobalPromise);
