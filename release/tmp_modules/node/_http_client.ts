...
// Sourced from src/js/node/_http_client.ts


const { isIP, isIPv6 } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 28/*internal/net/isIP*/) || __intrinsic__createInternalModuleById(28/*internal/net/isIP*/));
...
const nodeHttpClient = __intrinsic__lazy(39);
...
const { validateHeaderName } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 73/*node:_http_common*/) || __intrinsic__createInternalModuleById(73/*node:_http_common*/));
const { getTimerDuration } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 60/*internal/timers*/) || __intrinsic__createInternalModuleById(60/*internal/timers*/));
...
const { globalAgent } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 71/*node:_http_agent*/) || __intrinsic__createInternalModuleById(71/*node:_http_agent*/));
...
function ClientRequest(input, options, cb) {
  ...
  const write_ = (chunk, encoding, callback) => {
    ...
    const canSkipReEncodingData =
      ...
      // Buffer
      (__intrinsic__isTypedArrayView(chunk) && (!encoding || encoding === "buffer" || encoding === "utf-8"));
    let bodySize = 0;
    ...
    try {
      options.lookup(host, { all: true }, (err, results) => {
        ...
        const fail = (message, name, code, syscall) => {
          ...
          if (!!__intrinsic__debug) globalReportError(error);
  ...
  (IS_BUN_DEVELOPMENT?$debug_log(`new ClientRequest: ${this[kMethod]} ${this[kProtocol]}//${this[kHost]}:${this[kPort]}${this[kPath]}`):void 0);
